 ...
 */
function wpmcp_ajax_filter( $callback, $args = array(), $loop = false ) {
	...
	// Class Method callback
		...
		$errors->add( 'callback_error', __( 'An error occured when trying to perform the request: invalid callback or data.', 'wpmoviescore' ) );
 ...
 */
function wpmcp_ajax_response( $response, $i18n = array(), $nonce = null ) {

	if ( is_wp_error( $response ) )
		$_response = $response;
	else if ( ! is_object( $response ) && ! is_int( $response ) && ! is_array( $response ) && true !== $response )
		$_response = new WP_Error( 'callback_error', __( 'An error occured when trying to perform the request.', 'wpmoviescore' ) );
	else
		$_response = new WPMCP_Ajax( array( 'data' => $response, 'i18n' => $i18n, 'nonce' => $nonce ) );

	wpmcp_json_header( is_wp_error( $_response ) );
