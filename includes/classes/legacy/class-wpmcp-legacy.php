<?php
/**
 * WPMoviesCore Deprecated Meta Class.
 * 
 * This class handles deprecated WPMoviesCore Movie Metadata. Prior to WPMOLY
...
if ( ! class_exists( 'WPMCP_Legacy' ) ) :

	class WPMCP_Legacy extends WPMCP_Module {
		...
		public function init() {
			...
			if ( false === $total ) {
				update_option( 'wpmcp_has_deprecated_meta', '0' );
				return false;

			} else {
				$total = count( $total );
				update_option( 'wpmcp_has_deprecated_meta', $total );
			}
		}
		...
		public function register_hook_callbacks() {
			...
			add_action( 'wp_ajax_wpmcp_update_movie', array( $this, 'update_movie_callback' ) );
		}
		...
		public function update_movie_callback() {
			...
			wpmcp_check_ajax_referer( 'update-movie' );
			...
			wpmcp_ajax_response( $response, array(), wpmcp_create_nonce( 'update-movie' ) );
		}
				...
				$wpdb->prepare(
					"SELECT *, COUNT( meta_id ) AS m
					   ...
					   INNER JOIN {$wpdb->posts}
				   	   ON post_id = id
					  WHERE post_id = %d
					    AND meta_key IN ( '_wpmcp_movie_data', '_wpmcp_movie_tmdb_id', '_wpmcp_movie_title' )
					    AND meta_value != ''",
					$post_id
				)
			);
		 ...
		 * 
		 * Movie having an non-empty '_wpmcp_movie_data' custom field
		...
		public static function get_deprecated_movies() {
			...
			$movies = $wpdb->get_results(
				"SELECT DISTINCT post_id
				   ...
				   INNER JOIN {$wpdb->posts}
				   ON post_id = id
				  WHERE meta_key='_wpmcp_movie_data'
				    AND meta_value!=''
				    AND post_id NOT IN (
					SELECT DISTINCT post_id
					  FROM {$wpdb->postmeta}
					 WHERE ( meta_key='_wpmcp_movie_tmdb_id'
					      OR meta_key='_wpmcp_movie_title' )
					   AND meta_value!='' )
		...
		public static function update_movie( $movie_id ) {
			...
			if ( ! current_user_can( 'edit_post' ) )
				return new WP_Error( 'permission_denied', __( 'Error: you are not allowed to edit this movie.', 'wpmoviescore' ) );
		...
		private static function update_meta( $movie_id ) {
			...
			$meta = get_post_meta( $movie_id, '_wpmcp_movie_data', $single = true );
			if ( '' == $meta )
				return false;

			$update = WPMCP_Edit_Movies::save_movie_meta( $movie_id, $meta, $clean = false );

			if ( ! wpmcp_o( 'legacy-safety' ) && ! is_wp_error( $update ) && $update == $movie_id )
				delete_post_meta( $movie_id, '_wpmcp_movie_data', $meta );

			return $update;
		}

		/**
		 * Update old 'wpml' slug to new 'wpmcp' in database.
		 * 
		 * @since    2.0
		 */
		private static function update_slug() {
			...
			$update = $wpdb->query( "UPDATE {$wpdb->postmeta} SET meta_key=REPLACE(meta_key,'wpml_','wpmcp_') WHERE meta_id IN ({$movies})" );
		...
		public function activate( $network_wide ) {
			...
			self::update_slug();
			delete_option( 'wpmcp_has_deprecated_meta' );
			...
			add_option( 'wpmcp_has_deprecated_meta', $deprecated, null, 'no' );
		}
