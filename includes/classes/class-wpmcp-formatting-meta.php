<?php
/**
 * WPMoviesCore Utils Formatting_Meta Class extension.
 * 
...
if ( ! class_exists( 'WPMCP_Formatting_Meta' ) ) :
	...
	class WPMCP_Formatting_Meta {
		...
		public static function format_movie_director( $data ) {
			...
			$output = WPMCP_Utils::format_movie_terms_list( $data, 'collection' );
		...
		public static function format_movie_genres( $data ) {
			...
			$output = WPMCP_Utils::format_movie_terms_list( $data, 'genre' );
			$output = self::format_movie_field( $output );
		...
		public static function format_movie_cast( $data ) {
			...
			$output = WPMCP_Utils::format_movie_terms_list( $data,  'actor' );
			$output = self::format_movie_field( $output );
		...
		public static function format_movie_release_date( $data, $format = null ) {
			...
			if ( 'j F Y' == $format ) {
				...
				$year = apply_filters( 'wpmcp_movie_meta_link', array(
					'key'   => 'release_date',
					'value' => $year,
					'type'  => 'meta',
					'text'  => $year,
					'title' => sprintf( __( 'More movies released on %s', 'wpmoviescore' ), $year ),
				) );
				$date = apply_filters( 'wpmcp_movie_meta_link', array(
					'key'   => 'release_date',
					'value' => $_date,
					'type'  => 'meta',
					'text'  => $date,
					'title' => sprintf( __( 'More movies released on %s', 'wpmoviescore' ), date_i18n( 'F Y', strtotime( $data ) ) ),
				) );
				...
				$output = apply_filters( 'wpmcp_movie_meta_link', array(
					'key'   => 'release_date',
					'value' => $output,
					'type'  => 'meta',
					'text'  => $output,
					'title' => sprintf( __( 'More movies released on %s', 'wpmoviescore' ), $output ),
				) );
			}
		...
		public static function format_movie_runtime( $data, $format = null ) {
			...
			if ( '0' == $data )
				return self::format_movie_field( __( 'Duration unknown', 'wpmoviescore' ) );

			if ( is_null( $format ) )
				$format = wpmcp_o( 'format-time' );
		...
		public static function format_movie_spoken_languages( $data ) {
			...
			foreach ( $data as $i => $d ) {
				...
				$d = trim( $d );
				if ( '1' == wpmcp_o( 'translate-languages' ) ) {
					$title = WPMCP_L10n::get_language_standard_name( $d );
					$title = __( $title, 'wpmoviescore-iso' );
				} else {
					$title = $d;
				}

				$url = apply_filters( 'wpmcp_movie_meta_link', array(
					'key'   => 'spoken_languages',
					'value' => $d,
					'type'  => 'meta',
					'text'  => $title,
					'title' => sprintf( __( 'More movies in %s', 'wpmoviescore' ), $title )
				) );
				$data[ $i ] = $url;
			}
		...
		public static function format_movie_production_countries( $data ) {
			...
			if ( '1' == wpmcp_o( 'translate-countries' ) ) {
				$format = wpmcp_o( 'countries-format', array() );
			} else {
			...
			foreach ( $data as $i => $country ) {
				...
				foreach ( $format as $c => $f ) {
					...
					switch ( $f ) {
						case 'flag':
							$country = WPMCP_L10n::get_country_standard_name( $country );
							$code    = WPMCP_L10n::get_country_code( $country );
						...
						case 'translated':
							$country = WPMCP_L10n::get_country_standard_name( $country );
							$country = __( $country, 'wpmoviescore-iso' );
							$text    = $country;
							break;
						case 'ptranslated':
							$country = __( $country, 'wpmoviescore-iso' );
					...
					if ( 'flag' != $f && '' != $text ) {
						$text = apply_filters( 'wpmcp_movie_meta_link', array(
							'key'   => 'production_countries',
							'value' => $value,
							'type'  => 'meta',
							'text'  => $text,
							'title' => sprintf( __( 'More movies from country %s', 'wpmoviescore' ), $text )
						) );
					}
		...
		public static function format_movie_production_companies( $data ) {
			...
			foreach ( $data as $d ) {
				$output[] = apply_filters( 'wpmcp_movie_meta_link', array(
					...
					'type'  => 'meta',
					'text'  => $d,
					'title' => sprintf( __( 'More movies produced by %s', 'wpmoviescore' ), $d )
				) );
			}
		...
		public static function format_movie_composer( $data ) {
			...
			$output = apply_filters( 'wpmcp_movie_meta_link', array(
				...
				'type'  => 'meta',
				'text'  => $data,
				'title' => sprintf( __( 'More movies from composer %s', 'wpmoviescore' ), $data )
			) );
			$output = self::format_movie_field( $output );
		...
		public static function format_movie_editor( $data ) {
			...
			$output = apply_filters( 'wpmcp_movie_meta_link', array(
				...
				'type'  => 'meta',
				'text'  => $data,
				'title' => sprintf( __( 'More movies edited by %s', 'wpmoviescore' ), $data )
			) );
			$output = self::format_movie_field( $output );
		...
		public static function format_movie_author( $data ) {
			...
			$output = apply_filters( 'wpmcp_movie_meta_link', array(
				...
				'type'  => 'meta',
				'text'  => $data,
				'title' => sprintf( __( 'More movies from author %s', 'wpmoviescore' ), $data )
			) );
			$output = self::format_movie_field( $output );
		...
		public static function format_movie_photography( $data ) {
			...
			$output = apply_filters( 'wpmcp_movie_meta_link', array(
				...
				'type'  => 'meta',
				'text'  => $data,
				'title' => sprintf( __( 'More movies for director of photography %s', 'wpmoviescore' ), $data )
			) );
			$output = self::format_movie_field( $output );
		...
		public static function format_movie_certification( $data ) {
			...
			$output = apply_filters( 'wpmcp_movie_meta_link', array(
				...
				'type'  => 'meta',
				'text'  => $data,
				'title' => sprintf( __( 'More movies certified %s', 'wpmoviescore' ), $data )
			) );
			$output = self::format_movie_field( $output );
		...
		public static function format_movie_adult( $data ) {
			...
			$output = array( __( 'Yes', 'wpmoviescore' ), __( 'No', 'wpmoviescore' ) );
			...
			$output = apply_filters( 'wpmcp_movie_meta_link', array(
				...
				'type'  => 'meta',
				'text'  => $output,
				'title' => _n( 'More movies for adults only', 'More movies for all audiences', $output_, 'wpmoviescore' )
			) );
			$output = self::format_movie_field( $output );
		...
		public static function format_movie_homepage( $data ) {
			...
			if ( '' != $data )
				$data = sprintf( '<a href="%1$s" title="%2$s">%1$s</a>', $data, __( 'Official Website', 'wpmoviescore' ) );
		...
		public static function movie_country_flag( $code, $name ) {
			...
			if ( ! in_array( 'flag', wpmcp_o( 'countries-format' ) ) )
				return $name;
			 ...
			 */
			$flag = apply_filters( 'wpmcp_filter_country_flag_html', $flag, $code, $name );
