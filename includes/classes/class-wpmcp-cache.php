<?php
/**
 * WPMoviesCore Cache Class extension.
 * 
...
if ( ! class_exists( 'WPMCP_Cache' ) ) :
	 ...
	 * @author  Leo Gopal <leo@digitlab.co.za>
	 */
	class WPMCP_Cache extends WPMCP_Module {
		...
		public function __construct() {
			...
			if ( is_admin() || ( ! wpmcp_o( 'enable-cache' ) && ! wpmcp_o( 'cache-expire' ) ) )
				return false;
		}
		...
		public function register_hook_callbacks() {
			...
			add_filter( 'wpmcp_cache_name', __CLASS__ . '::wpmcp_cache_name', 10, 2 );
		}
		...
		public static function set( $transient, $value ) {
			...
			$set = set_transient( 'wpmcp_cache_' . $transient, $value );
		...
		public static function get( $transient ) {
			...
			return get_transient( 'wpmcp_cache_' . $transient );
		}
		...
		public static function delete( $transient ) {
			...
			return delete_transient( 'wpmcp_cache_' . $transient );
		}
		...
		public static function output( $name, $function, $echo = false ) {
			...
			if ( ! wpmcp_o( 'enable-cache' ) || ( is_user_logged_in() && ! wpmcp_o( 'cache-user' ) ) )
				return self::output_callback( $function, $echo );

			$expire = wpmcp_o( 'cache-expire' );
			if ( ! $expire )
				return false;

			$name = 'wpmcp_cache_' . $name;
		...
		public static function empty_cache() {
			...
			if ( false === $transient )
				return new WP_Error( 'transient_error', sprintf( __( 'An error occured when trying to delete transients: %s', 'wpmoviescore' ), $wpdb->last_error ) );
			else if ( ! $transient )
				return __( 'No transient found.', 'wpmoviescore' );
			else if ( $transient )
				return sprintf( _n( '1 transient deleted', '%s transients deleted.', $transient, 'wpmoviescore' ), $transient );
		}
		 ...
		 */
		public static function clean_transient( $action, $force = false, $search = 'wpmcp' ) {
			...
			if ( $force || ( ! $_wp_using_ext_object_cache && 'empty' == $action ) ) {
				...
				$where = '%_transient_' . wpmcp_esc_like( $where ) . '_%';
				// Restrict cleaning to Plugin's transients
				if ( false === stripos( $where, 'wpmcp' ) )
					$where = str_replace( '_transient_', '_transient_wpmcp%', $where );
		 ...
		 * @return   int        $result Number of deleted rows
		 */
		public static function wpmcp_cache_name( $name, $extra = null ) {
