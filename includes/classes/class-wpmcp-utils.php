<?php
/**
 * WPMoviesCore Utils Class extension.
 * 
...
if ( ! class_exists( 'WPMCP_Formatting_Meta' ) )
	require_once WPMCP_PATH . '/includes/classes/class-wpmcp-formatting-meta.php';
if ( ! class_exists( 'WPMCP_Formatting_Details' ) )
	require_once WPMCP_PATH . '/includes/classes/class-wpmcp-formatting-details.php';

if ( ! class_exists( 'WPMCP_Utils' ) ) :

	class WPMCP_Utils extends WPMCP_Module {
		...
		public function init() {
			...
			$methods = array(
				'WPMCP_Formatting_Meta'    => get_class_methods( 'WPMCP_Formatting_Meta' ),
				'WPMCP_Formatting_Details' => get_class_methods( 'WPMCP_Formatting_Details' )
			);
		...
		public function register_hook_callbacks() {
			...
			add_filter( 'wpmcp_filter_meta_data', __CLASS__ . '::filter_meta_data', 10, 1 );
			add_filter( 'wpmcp_filter_crew_data', __CLASS__ . '::filter_crew_data', 10, 1 );
			add_filter( 'wpmcp_filter_cast_data', __CLASS__ . '::filter_cast_data', 10, 1 );
			add_filter( 'wpmcp_filter_movie_meta_aliases', __CLASS__ . '::filter_movie_meta_aliases', 10, 1 );

			foreach ( $this->filters as $filter )
				add_filter( $filter['tag'], $filter['function'], $filter['priority'], $filter['args'] );

			add_filter( 'wpmcp_movie_meta_link', __CLASS__ . '::add_meta_link', 10, 1 );

			add_filter( 'wpmcp_movie_rating_stars', __CLASS__ . '::get_movie_rating_stars', 10, 4 );
		...
		private function has_permalinks_changed() {
			...
			$changed = get_transient( 'wpmcp-permalinks-changed' );
			if ( false === $changed )
				return false;
		...
		public static function permalinks_changed( $field, $value, $existing_value ) {
			...
			$rewrites = array(
				'wpmcp-rewrite-movie',
				'wpmcp-rewrite-collection',
				'wpmcp-rewrite-genre',
				'wpmcp-rewrite-actor',
				'wpmcp-movie-archives',
				'wpmcp-collection-archives',
				'wpmcp-genre-archives',
				'wpmcp-actor-archives'
			);
			...
			$changed = set_transient( 'wpmcp-permalinks-changed', $field['id'] );
		...
		public static function register_permalinks( $rules = null ) {
			...
			$changed = delete_transient( 'wpmcp-permalinks-changed' );
		...
		private static function generate_custom_rules( $rules ) {
			...
			WPMCP_L10n::delete_l10n_rewrite();
			WPMCP_L10n::delete_l10n_rewrite_rules();
			$l10n_rules = WPMCP_L10n::set_l10n_rewrite_rules();
			...
			$archives = array(
				'movie'      => intval( wpmcp_o( 'movie-archives' ) ),
				'collection' => intval( wpmcp_o( 'collection-archives' ) ),
				'genre'      => intval( wpmcp_o( 'genre-archives' ) ),
				'actor'      => intval( wpmcp_o( 'actor-archives' ) )
			);
			extract( $archives );
			...
			if ( '1' == $translate )
				$grid = sprintf( '(%s|%s|%s)', __( 'grid', 'wpmoviescore' ), __( 'archives', 'wpmoviescore' ), __( 'list', 'wpmoviescore' ) );
		 ...
		 * @param    string    $type Notice type: update, error, wpmcp?
		 */
		public static function admin_notice( $notice, $type = 'update' ) {
			...
			if ( ! in_array( $type, array( 'error', 'warning', 'update', 'wpmoviescore' ) ) || 'update' == $type )
				$class = 'updated';
			else if ( 'wpmcp' == $type )
				$class = 'updated wpmcp';
			else if ( 'error' == $type )
		...
		public static function filter_meta_data( $data ) {
			...
			$filtered_meta = array();
			$supported_meta = WPMCP_Settings::get_supported_movie_meta( 'meta' );
		...
		public static function filter_crew_data( $data ) {
			...
			$supported_jobs = WPMCP_Settings::get_supported_movie_meta( 'crew' );
			...
			$filtered_crew['cast'] = apply_filters( 'wpmcp_filter_cast_data', $data['cast'] );
		...
		public static function filter_movie_meta_aliases( $slug ) {
			...
			$aliases = WPMCP_Settings::get_supported_movie_meta_aliases();
		...
		public static function add_meta_link( $args ) {
			...
			if ( ! wpmcp_o( 'meta-links' ) || 'nowhere' == wpmcp_o( 'meta-links' ) || ( 'posts_only' == wpmcp_o( 'meta-links' ) && ! is_single() ) )
				return $text;
		...
		public static function build_meta_permalink( $args ) {
			...
			if ( '1' == wpmcp_o( 'rewrite-enable' ) ) {
				if ( 'production_countries' == $args['meta'] ) {
					$args['value'] = WPMCP_L10n::get_country_standard_name( $args['value'] );
				} else if ( 'spoken_languages' == $args['meta'] ) {
					$args['value'] = WPMCP_L10n::get_language_standard_name( $args['value'] );
				}
				$args['value'] = __( $args['value'], 'wpmoviescore-iso' );
			}

			if ( 'rating' != $args['meta'] )
				$args['value'] = sanitize_title( $args['value'] );

			$args['meta'] = WPMCP_L10n::translate_rewrite( $args['meta'] );
			$args['value'] = WPMCP_L10n::translate_rewrite( $args['value'] );
		...
		private static function build_default_meta_permalink( $args ) {
			...
			if ( ! isset( $args['baseurl'] ) || empty( $args['baseurl'] ) ) {
				$page = intval( wpmcp_o( $type . '-archives' ) );
				$base  = 'index.php?';
				if ( $page )
					$base .= 'page_id=' . $page . '&';
				$base = home_url( "/${base}" );
		...
		public static function get_taxonomy_permalink( $taxonomy, $value ) {
			...
			$page_id = intval( wpmcp_o( "{$taxonomy}-archives" ) );
		...
		public static function get_movie_rating_stars( $rating, $post_id = null, $base = null, $include_empty = false ) {
			...
			$defaults = WPMCP_Settings::get_supported_movie_details();

			if ( is_null( $post_id ) || ! intval( $post_id ) )
				$post_id = get_the_ID();

			if ( is_null( $base ) )
				$base = wpmcp_o( 'format-rating' );
			if ( 10 != $base )
				$base = 5;
			...
			$class   = "wpmcp-movie-rating wpmcp-movie-rating-{$_rating}";
			...
			if ( 0.0 == $rating ) {
				if ( true === $include_empty ) {
					$stars  = '<div id="wpmcp-movie-rating-' . $post_id . '" class="' . $class . '" title="' . $title . '">';
					$stars .= str_repeat( $empty, 10 );
					$stars .= '</div>';
				} else {
					$stars  = '<div id="wpmcp-movie-rating-' . $post_id . '" class="not-rated" title="' . $title . '">';
					$stars .= sprintf( '<small><em>%s</em></small>', __( 'Not rated yet!', 'wpmoviescore' ) );
					$stars .= '</div>';
				}
			}
			else if ( 10 == $base ) {
			 ...
			 */
			$stars = apply_filters( 'wpmcp_movie_rating_stars_html', $stars, $rating );
		...
		public static function format_movie_terms_list( $data, $taxonomy ) {
			...
			$has_taxonomy = wpmcp_o( "enable-{$taxonomy}" );
			$_data = explode( ',', $data );
			...
			foreach ( $_data as $key => $term ) {
				...
				if ( is_object( $_term ) && '' != $_term->name ) {
					...
					$_term = ( is_wp_error( $link ) ? $_term->name : sprintf( '<a href="%s" title="%s">%s</a>', $link, sprintf( __( 'More movies from %s', 'wpmoviescore' ), $_term->name ), $_term->name ) );
				}
				$_data[ $key ] = $_term;
			}
		...
		public static function get_used_years( $count = false ) {
			...
			foreach ( $used_years as $i => $year ) {
				...
				if ( ! $count )
					...
					$used_years[ $year['name'] ] = sprintf( '%s (%s)', $year['name'], sprintf( _n( '%d movie', '%d movies', $year['count'], 'wpmoviescore' ), $year['count'] ) );
			}
		...
		public static function get_used_languages( $count = false ) {
			...
			foreach ( $languages as $i => $language ) {
				$_language = WPMCP_L10n::get_language_standard_name( $language['name'] );
				if ( ! $count ) {
					$used_languages[ $language['name'] ] = __( $_language, 'wpmoviescore-iso' );
				} else {
					$used_languages[ $language['name'] ] = sprintf( '%s (%s)', __( $_language, 'wpmoviescore-iso' ), sprintf( _n( '%d movie', '%d movies', $language['count'], 'wpmoviescore' ), $language['count'] ) );
				}
			}
		...
		public static function get_used_countries( $count = false ) {
			...
			foreach ( $countries as $i => $country ) {
				$_country = WPMCP_L10n::get_country_standard_name( $country['name'] );
				if ( ! $count ) {
					$used_countries[ $country['name'] ] = __( $_country, 'wpmoviescore-iso' );
				} else {
					$used_countries[ $country['name'] ] = sprintf( '%s (%s)', __( $_country, 'wpmoviescore-iso' ), sprintf( _n( '%d movie', '%d movies', $country['count'], 'wpmoviescore' ), $country['count'] ) );
				}
			}
		...
		public static function get_used_companies( $count = false ) {
			...
			foreach ( $companies as $i => $company ) {
				if ( ! $count )
					...
					$used_companies[ $company['name'] ] = sprintf( '%s (%s)', $company['name'], sprintf( _n( '%d movie', '%d movies', $company['count'], 'wpmoviescore' ), $company['count'] ) );
			}
		...
		public static function get_used_certifications( $count = false ) {
			...
			foreach ( $certifications as $i => $certification ) {
				if ( ! $count )
					...
					$used_certification[ $certification['name'] ] = sprintf( '%s (%s)', $certification['name'], sprintf( _n( '%d movie', '%d movies', $certification['count'], 'wpmoviescore' ), $certification['count'] ) );
			}
		...
		public static function get_used_meta( $meta, $count = false ) {
			...
			$supported = array_keys( WPMCP_Settings::get_supported_movie_meta() );
			...
			$values = $wpdb->get_results(
				"SELECT {$distinct} meta_value
				 FROM {$wpdb->postmeta}
				 WHERE meta_key LIKE '_wpmcp_movie_{$meta}'
				 ORDER BY meta_value"
			);
		...
		public static function filter_default_thumbnail( $html, $post_id, $post_thumbnail_id, $size, $attr ) {
			...
			$url = str_replace( '{size}', $size, WPMCP_DEFAULT_POSTER_URL );
		...
		*/
		public static function paginate_links( $args = '' ) {
			...
			$content = WPMoviesCore::render_template( 'archives/pagination.php', $attributes, $require = 'always' );

			return $content;
		}

		/**
		 * Deactivate all child plugins when deactivating WPMoviesCore
		 * to avoid errors.
		 *
		 * @since    2.0
		 *
		 ...
		 */
		public static function wpmcp_required() {

			$chidren = array();
			$wpmcp  = WPMCP_PLUGIN;
			$parent  = plugin_basename( WPMCP_PATH );
			$plugins = get_plugins();

			foreach ( $plugins as $slug => $plugin ) {
				if ( false !== stripos( $slug, $parent ) && $slug != $wpmcp && is_plugin_active( $slug ) ) {
			...
			if ( ! empty( $chidren ) )
				wp_die( sprintf( __( '<strong>%s</strong> is required by the following plugins: %s. These plugins have been deactivated to prevent errors.<br /><br />Back to the WordPress <a href="%s">Plugins page</a>.', 'wpmoviescore' ), WPMCP_NAME, implode( ', ', $chidren ), admin_url( 'plugins.php' ) ) );
		}
		...
		public function activate( $network_wide ) {
			...
			// About page
			update_option( '_wpmcp_fresh_install', 'yes' );
		}
		...
		public function deactivate() {
			...
			WPMCP_Cache::clean_transient( 'deactivate' );
		...
		public static function uninstall() {
			...
			WPMCP_Cache::clean_transient( 'uninstall' );
			delete_option( 'rewrite_rules' );
		}
