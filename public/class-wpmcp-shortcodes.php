<?php
/**
 * WPMoviesCore Shortcodes Class extension.
 *
...
require_once WPMCP_PATH . '/includes/config/wpmcp-shortcodes.php';

if ( ! class_exists( 'WPMCP_Shortcodes' ) ) :

	class WPMCP_Shortcodes extends WPMCP_Module {
		...
		public function init() {
			...
			$this->shortcodes = WPMCP_Settings::get_available_shortcodes();
		...
		public static function movie_grid_shortcode( $atts = array(), $content = null ) {
			...
			if ( 'page' != get_post_type() ) {
				...
				if ( current_user_can( 'manage_options' ) )
					$notive = sprintf( '<p><em>%s</em></p>', sprintf( __( 'The Movie Grid can not be used on regular Posts (<a href="%s">learn why</a>), please consider using a Page instead. This message is visible by logged in administrators only.', 'wpmoviescore' ), esc_url( 'http://wpmoviescore.com/documentation/shortcodes/movie_grid/' ) ) );

				return $notive;
			}

			// Caching
			$name = apply_filters( 'wpmcp_cache_name', 'movie_grid_shortcode', $atts );
			$content = WPMCP_Cache::output( $name, function() use ( $atts, $content ) {

				extract( $atts );

				$grid_menu = '';
				if ( $menu )
					$grid_menu = WPMCP_Grid::get_menu( $atts, $shortcode = true );

				$content = WPMCP_Grid::get_content( $atts, $shortcode = true );
		...
		public static function movies_shortcode( $atts = array(), $content = null ) {
			...
			$default_fields = WPMCP_Settings::get_supported_movie_meta();
			$atts = self::filter_shortcode_atts( 'movies', $atts );

			// Caching
			$name = apply_filters( 'wpmcp_cache_name', 'movies_shortcode', $atts );
			$content = WPMCP_Cache::output( $name, function() use ( $atts, $content ) {
				...
				if ( ! is_null( $orderby ) ) {
					if ( 'rating' == $orderby ) {
						...
						$query[] = 'meta_key=_wpmcp_movie_rating';
					}
					else {
				...
				$movies = WPMCP_Shortcodes::prepare_movies( $query, $atts );
				...
				$paginate = WPMCP_Utils::paginate_links( $args );
				$paginate = '<div id="wpmcp-movies-pagination">' . $paginate . '</div>';
	
				$content = WPMoviesCore::render_template( 'shortcodes/movies.php', $attributes, $require = 'always' );
		...
		public static function movie_shortcode( $atts = array(), $content = null ) {
			...
			$movie_id = WPMCP_Shortcodes::find_movie_id( $atts['id'], $atts['title'] );
			if ( is_null( $movie_id ) )
				return $content;
			...
			// Caching
			$name = apply_filters( 'wpmcp_cache_name', 'movie_shortcode', $atts );
			$content = WPMCP_Cache::output( $name, function() use ( $atts, $content ) {
		...
		public static function movie_meta_shortcode( $atts = array(), $content = null, $tag = null ) {
			...
			if ( ! is_null( $tag ) && "{$tag}_shortcode" != __FUNCTION__ ) {
				$tag = apply_filters( 'wpmcp_filter_movie_meta_aliases', $tag );
			...
			// Caching
			$name = apply_filters( 'wpmcp_cache_name', 'movie_meta_shortcode', $atts );
			$content = WPMCP_Cache::output( $name, function() use ( $atts, $content ) {

				extract( $atts );

				$_meta = WPMCP_Settings::get_supported_movie_meta();
				$alias = WPMCP_Settings::get_supported_movie_meta_aliases();
				if ( ! isset( $_meta[ $key ] ) && isset( $alias[ $key ] ) )
					$key = $alias[ $key ];

				$meta = wpmcp_get_movie_meta( $movie_id, $key );
				$meta = apply_filters( "wpmcp_format_movie_$key", $meta );
				$meta = apply_filters( "wpmcp_format_movie_field", $meta );

				$view = 'shortcodes/metadata.php';
				$attributes = array( 'key' => $key, 'meta' => $meta );
				if ( $label ) {
					$attributes['title'] = __( $_meta[ $key ]['title'], 'wpmoviescore' );
					$view = 'shortcodes/metadata-label.php';
				}

				$content = WPMoviesCore::render_template( $view, $attributes, $require = 'always' );
		...
		public static function movie_release_date_shortcode( $atts = array(), $content = null, $tag = null ) {
			...
			// Caching
			$name = apply_filters( 'wpmcp_cache_name', 'movie_release_date_shortcode', $atts );
			$content = WPMCP_Cache::output( $name, function() use ( $atts, $content ) {

				extract( $atts );

				$release_date = wpmcp_get_movie_meta( $movie_id, 'release_date' );
				$release_date = apply_filters( "wpmcp_format_movie_release_date", $release_date, $format );

				$view = 'shortcodes/metadata.php';
				$attributes = array( 'meta' => $release_date );
				if ( $label ) {
					$attributes['title'] = __( 'Release Date', 'wpmoviescore' );
					$view = 'shortcodes/metadata-label.php';
				}
		...
		public static function movie_local_release_date_shortcode( $atts = array(), $content = null, $tag = null ) {
			...
			// Caching
			$name = apply_filters( 'wpmcp_cache_name', 'movie_local_release_date_shortcode', $atts );
			$content = WPMCP_Cache::output( $name, function() use ( $atts, $content ) {

				extract( $atts );

				$release_date = wpmcp_get_movie_meta( $movie_id, 'local_release_date' );
				$release_date = apply_filters( "wpmcp_format_movie_release_date", $release_date, $format );

				$view = 'shortcodes/metadata.php';
				$attributes = array( 'meta' => $release_date );
				if ( $label ) {
					$attributes['title'] = __( 'Local Release Date', 'wpmoviescore' );
					$view = 'shortcodes/metadata-label.php';
				}
		...
		public static function movie_runtime_shortcode( $atts = array(), $content = null ) {
			...
			// Caching
			$name = apply_filters( 'wpmcp_cache_name', 'movie_runtime_shortcode', $atts );
			$content = WPMCP_Cache::output( $name, function() use ( $atts, $content ) {

				extract( $atts );

				$runtime = wpmcp_get_movie_meta( $movie_id, 'runtime' );
				$runtime = apply_filters( "wpmcp_format_movie_runtime", $runtime, $format );

				$view = 'shortcodes/metadata.php';
				$attributes = array( 'meta' => $runtime );
				if ( $label ) {
					$attributes['title'] = __( 'Runtime', 'wpmoviescore' );
					$view = 'shortcodes/metadata-label.php';
				}
		...
		public static function movie_actors_shortcode( $atts = array(), $content = null, $tag = null ) {
			...
			// Caching
			$name = apply_filters( 'wpmcp_cache_name', 'movie_actors_shortcode', $atts );
			$content = WPMCP_Cache::output( $name, function() use ( $atts, $content ) {

				extract( $atts );

				$actors = wpmcp_get_movie_meta( $movie_id, 'cast' );
				$actors = apply_filters( "wpmcp_format_movie_actors", $actors );
				...
				if ( $label ) {
					$attributes['title'] = __( 'Actors', 'wpmoviescore' );
		...
		public static function movie_genres_shortcode( $atts = array(), $content = null ) {
			...
			// Caching
			$name = apply_filters( 'wpmcp_cache_name', 'movie_genres_shortcode', $atts );
			$content = WPMCP_Cache::output( $name, function() use ( $atts, $content ) {

				extract( $atts );

				$genres = wpmcp_get_movie_meta( $movie_id, 'genres' );
				$genres = apply_filters( "wpmcp_format_movie_genres", $genres, $movie_id );
				...
				if ( $label ) {
					$attributes['title'] = __( 'Genres', 'wpmoviescore' );
		...
		public static function movie_poster_shortcode( $atts = array(), $content = null ) {
			...
			// Caching
			$name = apply_filters( 'wpmcp_cache_name', 'movie_posters_shortcode', $atts );
			$content = WPMCP_Cache::output( $name, function() use ( $atts, $content ) {
				...
				$content = WPMoviesCore::render_template( 'shortcodes/poster.php', $attributes, $require = 'always' );
		...
		public static function movie_images_shortcode( $atts = array(), $content = null, $tag = null ) {
			...
			// Caching
			$name = apply_filters( 'wpmcp_cache_name', 'movie_images_shortcode', $atts );
			$content = WPMCP_Cache::output( $name, function() use ( $atts, $content ) {
				...
				if ( 'posters' == $type ) {
					$args = array(
						'meta_key' => '_wpmcp_poster_related_tmdb_id'
					);
				} elseif ( 'images' == $type || 'backdrops' == $type ) {
					$args = array(
						'meta_key' => '_wpmcp_image_related_tmdb_id'
					);
				} else {
					$args = array(
				...
				if ( ! empty( $args ) ) {
					...
					if ( $attachments ) {
						...
						$content = WPMoviesCore::render_template( 'shortcodes/images.php', array( 'size' => $size, 'movie_id' => $movie_id, 'images' => $images ), $require = 'always' );
					}
				}
		...
		public static function movie_rating_shortcode( $atts = array(), $content = null ) {
			...
			// Caching
			$name = apply_filters( 'wpmcp_cache_name', 'movie_rating_shortcode', $atts );
			$content = WPMCP_Cache::output( $name, function() use ( $atts, $content ) {

				extract( $atts );

				$content = wpmcp_get_movie_rating( $movie_id );

				if ( $stars || $numbers ) {

					$output = '';
					if ( $stars )
						$output .= apply_filters( 'wpmcp_format_movie_rating', $content, $format = 'html' );

					if ( $numbers ) {
						$output .= '&nbsp;−&nbsp;';
						if ( 10 == wpmcp_o( 'format-rating' ) ) {
					...
					$format = ( ! $raw ? 'html' : 'raw' );
					$output = apply_filters( 'wpmcp_format_movie_rating', $content, $format );
				}
		...
		public static function movie_detail_shortcode( $atts = array(), $content = null, $tag = null ) {
			...
			// Caching
			$name = apply_filters( 'wpmcp_cache_name', 'movie_detail_shortcode', $atts );
			$content = WPMCP_Cache::output( $name, function() use ( $atts, $content ) {

				extract( $atts );

				if ( ! function_exists( "wpmcp_get_movie_{$key}" ) )
					return $content;

				$content = call_user_func( "wpmcp_get_movie_{$key}", $movie_id );

				$format = ( ! $raw ? 'html' : 'raw' );
				$content = apply_filters( "wpmcp_format_movie_{$key}", $content, $format );
		...
		private static function prepare_movies( $query, $atts ) {
			...
			while ( $query->have_posts() ) {
				...
				if ( ! is_null( $meta ) ) {
					...
					$_meta = wpmcp_get_movie_meta( get_the_ID(), 'data' );
					$_meta = wpmcp_filter_undimension_array( $_meta );

					$metadata = array();

					foreach ( $_meta as $slug => $m ) {
						if ( in_array( $slug, $meta ) ) {
							$title = __( $default_fields[ $slug ]['title'], 'wpmoviescore' );
							$value = $_meta[ $slug ];
							if ( has_filter( "wpmcp_format_movie_{$slug}" ) )
								$value = apply_filters( "wpmcp_format_movie_{$slug}", $value );
							$value = apply_filters( 'wpmcp_format_movie_field', $value );
				...
				if ( ! is_null( $details ) ) {
					...
					foreach ( $details as $detail ) {
						$value = call_user_func( "wpmcp_get_movie_{$detail}", get_the_ID() );
						$movies[ $query->current_post ]['details'] .= apply_filters( "wpmcp_format_movie_$detail", $value );
					}
				}
			}
			wp_reset_postdata();
		...
		public static function filter_shortcode_atts( $shortcode, $atts = array(), $defaults = array() ) {
			...
			if ( empty( $defaults ) ) {
				$defaults = WPMCP_Settings::get_available_shortcodes();
