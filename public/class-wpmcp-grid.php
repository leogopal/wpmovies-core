<?php
/**
 * WPMoviesCore Movie Grid Class extension.
 *
...
if ( ! class_exists( 'WPMCP_Grid' ) ) :

	class WPMCP_Grid extends WPMCP_Movies {
		...
		public static function get_menu( $args, $shortcode = false ) {
			...
			$defaults = array(
				'order'    => wpmcp_o( 'movie-archives-movies-order', $default = true ),
				'orderby'  => wpmcp_o( 'movie-archives-movies-orderby', $default = true ),
				'columns'  => wpmcp_o( 'movie-archives-grid-columns', $default = true ),
				'rows'     => wpmcp_o( 'movie-archives-grid-rows', $default = true ),
				'editable' => wpmcp_o( 'movie-archives-frontend-edit', $default = true ),
				'advanced' => wpmcp_o( 'movie-archives-frontend-advanced-edit', $default = false ),
				'meta'     => '',
				'detail'   => '',
				'value'    => '',
				'letter'   => '',
			...
			$_args = WPMCP_Archives::parse_query_vars( $wp_query->query );
			...
			if ( '1' == wpmcp_o( 'rewrite-enable' ) )
				$views = array( 'grid' => __( 'grid', 'wpmoviescore' ), 'archives' => __( 'archives', 'wpmoviescore' ), 'list' => __( 'list', 'wpmoviescore' ) );
			...
			$limit = wpmcp_o( 'movie-archives-movies-limit' );
			...
			$args['letter'] = $letter;
			$args['view'] = 'list';
			$urls['list'] = WPMCP_Utils::build_meta_permalink( $args );
			$args['view'] = 'archives';
			$urls['archives'] = WPMCP_Utils::build_meta_permalink( $args );
			$args['view'] = 'grid';
			$urls['grid'] = WPMCP_Utils::build_meta_permalink( $args );
			$args['view'] = $view;
			...
			$args['order'] = 'ASC';
			$urls['asc'] = WPMCP_Utils::build_meta_permalink( $args );

			$args['order'] = 'DESC';
			$urls['desc'] = WPMCP_Utils::build_meta_permalink( $args );
		...
		public static function get_content( $args = array(), $shortcode = false ) {
			...
			$grid_meta = (array) wpmcp_o( 'movie-archives-movies-meta', $default = true );
			...
			$number = $columns * $rows;
			$limit = wpmcp_o( 'movie-archives-movies-limit', $default = true );
			if ( -1 == $number )
				$number = $limit;
			...
			if ( '' != $letter )
				$where[] = " AND post_title LIKE '" . wpmcp_esc_like( $letter ) . "%'";

			$join = array();
			$meta_query = array( 'join' => array(), 'where' => array() );
			if ( '' != $value && '' != $meta ) {

				$meta_query = call_user_func( "WPMCP_Search::by_$meta", $value, 'sql' );
			...
			if ( in_array( $orderby, array_keys( $_orderby ) ) ) {
				...
				$where[]  = ' AND pm.meta_key = "_wpmcp_movie_' . $_orderby[ $orderby ] . '"';
				$orderby = 'value';
			...
			// debug
			$debug = null;
			if ( current_user_can( 'manage_options' ) && '1' == wpmcp_o( 'debug-mode' ) ) {
