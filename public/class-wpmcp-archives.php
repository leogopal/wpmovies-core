<?php
/**
 * WPMoviesCore Archives Class extension.
 * 
...
if ( ! class_exists( 'WPMCP_Archives' ) ) :

	class WPMCP_Archives extends WPMCP_Module {
		...
		public function custom_pages_notice() {
			...
			if ( 'yes' == get_option( 'wpmcp_has_custom_pages', 'no' ) )
				return false;
		...
		public static function hide_custom_pages_notice() {
			...
			update_option( 'wpmcp_has_custom_pages', 'yes' );
		}
		...
		public function template_redirect() {
			...
			$page = intval( wpmcp_o( 'movie-archives' ) );
		...
		public static function create_pages() {
			...
			if ( isset( $_GET['_wpmcpnonce_dismiss_custom_pages_notice'] ) )
			...
			foreach ( $pages as $slug => $page ) {
				...
				if ( ! $page ) {
					...
					$missing[ $slug ] = __( $title, 'wpmoviescore' );
				}
				else {
		...
		public static function add_custom_pages() {
			...
			$nonce = '_wpmcpnonce_create_custom_pages';
			if ( ! isset( $_GET[ $nonce ] ) || ! wpmcp_verify_nonce( $_GET[ $nonce ], 'create-custom-pages' ) ) {
			...
			switch ( $create ) {
				case 'all':
					$pages = array(
						'movie'      => __( 'Movies', 'wpmoviescore' ),
						'collection' => __( 'Collections', 'wpmoviescore' ),
						'genre'      => __( 'Genres', 'wpmoviescore' ),
						'actor'      => __( 'Actors', 'wpmoviescore' ),
					);
					break;
				case 'movie':
					$pages = array( 'movie' => __( 'Movies', 'wpmoviescore' ) );
					break;
				case 'collection':
					$pages = array( 'collection' => __( 'Collections', 'wpmoviescore' ) );
					break;
				case 'genre':
					$pages = array( 'genre' => __( 'Genres', 'wpmoviescore' ) );
					break;
				case 'actor':
					$pages = array( 'actor' => __( 'Actors', 'wpmoviescore' ) );
					break;
				default:
					$pages = array();
					break;
			...
			foreach ( $pages as $slug => $page ) {
				...
				$exists = intval( wpmcp_o( "$slug-archives" ) );
				if ( ! $exists ) {
					...
					if ( $page )
						$wpmcp_redux_config->ReduxFramework->set( "wpmcp-$slug-archives", $page );
				}
			}

			$self = new WPMCP_Archives();
		...
		public function get_pages( $content ) {
			...
			if ( $movie && $archive ) {
				$position = wpmcp_o( 'movie-archives-position' );
			} else if ( ( $collection || $genre || $actor ) && $archive ) {
				$position = wpmcp_o( 'tax-archives-position' );
			} else {
				$position = 'top';
			}
		...
		public function movie_archives_title( $title, $id = null ) {
			...
			$translate = ( '1' == wpmcp_o( 'rewrite-enable' ) );
			$rewrite_movie = ( '1' == wpmcp_o( 'movie-archives-title-rewrite' ) );
			$rewrite_taxonomy = ( '1' == wpmcp_o( 'tax-archives-title-rewrite' ) );
			 ...
			 */
			$titles = apply_filters( 'wpmcp_filter_archive_pages_titles', $titles );
			 ...
			 */
			$title = apply_filters( "wpmcp_filter_{$page}_archive_page_default_title", $titles[ $page ], $id );
			...
			if ( $is_movie && $rewrite_movie ) {
				...
				if ( '' != $meta ) {
					...
					if ( $translate )
						$meta = WPMCP_L10n::untranslate_rewrite( $meta );
					...
					if ( '' != $value )
						$value = WPMCP_L10n::untranslate_rewrite( $value );
				}
				elseif ( '' != $detail ) {

					if ( $translate )
						$detail = WPMCP_L10n::untranslate_rewrite( $detail );

					$supported = WPMCP_Settings::get_supported_movie_detail();
				...
				if ( 'production_countries' == $meta ) {
					$value = WPMCP_L10n::get_country_standard_name( $value );
					$value = __( $value, 'wpmoviescore-iso' );
				} else if ( 'spoken_languages' == $meta ) {
					$value = WPMCP_L10n::get_language_standard_name( $value );
					$value = __( $value, 'wpmoviescore-iso' );
				} else {
					$value = __( $value, 'wpmoviescore' );
				}
				...
				if ( '' != $_meta && '' == $value )
					$title = sprintf( __( 'Movies by %s', 'wpmoviescore' ), $_meta );
				elseif ( '' != $_meta && '' != $value )
					$title = sprintf( __( 'Movies by %s: %s', 'wpmoviescore' ), ucwords( $_meta ), $value );
				else
			...
			if ( '' != $sorting ) {
				...
				if ( isset( $sorting['letter'] ) && '' != $sorting['letter'] )
					$title  .= sprintf( ' − %s ', sprintf( __( 'Letter %s', 'wpmoviescore' ), $sorting['letter'] ) );

				if ( isset( $sorting['paged'] ) && '' != $sorting['paged'] && 'wp_title' == $filter )
					$title .= sprintf( __( ' %s Page %d ', 'wpmoviescore' ), $sep, $sorting['paged'] );
			}
			 ...
			 * @return   string    Filtered title
			 */
			$title = apply_filters( "wpmcp_filter_{$page}_archive_page_title", $title, $id );
			...
			if ( 'wp_title' == $filter ) {
				 ...
				 * @return   string    Filtered title
				 */
				$title = apply_filters( "wpmcp_filter_{$page}_archive_page_wp_title", $title, $id );
			}
		...
		public static function movie_archives() {
			...
			$has_menu = wpmcp_o( 'movie-archives-menu', $default = true );
			$editable = wpmcp_o( 'movie-archives-frontend-edit', $default = true );
		...
		public static function taxonomy_archives( $taxonomy ) {
			...
			if ( 'collection' == $taxonomy )
				$term_title = __( 'View all movies from collection &laquo; %s &raquo;', 'wpmoviescore' );
			else if ( 'genre' == $taxonomy )
				$term_title = __( 'View all &laquo; %s &raquo; movies', 'wpmoviescore' );
			else if ( 'actor' == $taxonomy )
				$term_title = __( 'View all movies staring &laquo; %s &raquo;', 'wpmoviescore' );
			...
			$name = WPMCP_Cache::wpmcp_cache_name( "{$taxonomy}_archive" );
			$content = WPMCP_Cache::output( $name, function() use ( $wpdb, $taxonomy, $term_title, $params ) {

				$has_menu = wpmcp_o( 'tax-archives-menu', $default = true );
				$hide_empty = wpmcp_o( 'tax-archives-hide-empty', $default = true );
				...
				$number = min( $number, wpmcp_o( 'tax-archives-terms-limit', $default = true ) );
				if ( ! $number )
					$number = wpmcp_o( 'tax-archives-terms-per-page', $default = true );
				...
				if ( is_wp_error( $terms ) )
					...
					foreach ( $terms as $term )
						$links[] = array(
							...
							'title'      => $term->name,
							'count'      => sprintf( _n( '%d movie', '%d movies', $term->count, 'wpmoviescore' ), $term->count )
						);
				...
				if ( $has_menu ) {
					...
					// PHP 5.3
					$menu = WPMCP_Archives::taxonomy_archive_menu( $taxonomy, $args );
				}

				$args['letter'] = $letter;
				$args['baseurl'] = get_permalink();
				$url = WPMCP_Utils::build_meta_permalink( $args );
				...
				$pagination = WPMCP_Utils::paginate_links( $args );
				$pagination = '<div id="wpmcp-movies-pagination">' . $pagination . '</div>';

				$attributes = array( 'taxonomy' => $taxonomy, 'links' => $links );
				$content = WPMoviesCore::render_template( 'archives/archives.php', $attributes, $require = 'always' );
		...
		public static function taxonomy_archive_menu( $taxonomy, $args ) {
			...
			$defaults = array(
				'letter'   => '',
				'order'    => wpmcp_o( 'tax-archives-terms-order', $default = true ),
				'orderby'  => wpmcp_o( 'tax-archives-terms-orderby', $default = true ),
				'number'   => wpmcp_o( 'tax-archives-terms-per-page', $default = true ),
				'editable' => wpmcp_o( 'tax-archives-frontend-edit', $default = true )
			);
			...
			$urls = array();
			$urls['all'] = WPMCP_Utils::build_meta_permalink( $args );
			$args['order'] = $order;

			$args['letter'] = ':letter:';
			$urls['letter'] = WPMCP_Utils::build_meta_permalink( $args );
			$args['letter'] = $letter;

			$args['order'] = 'ASC';
			$args['orderby'] = 'count';
			$urls['count_asc'] = WPMCP_Utils::build_meta_permalink( $args );
			$args['orderby'] = 'title';
			$urls['title_asc'] = WPMCP_Utils::build_meta_permalink( $args );

			$args['order'] = 'DESC';
			$args['orderby'] = 'count';
			$urls['count_desc'] = WPMCP_Utils::build_meta_permalink( $args );
			$args['orderby'] = 'title';
			$urls['title_desc'] = WPMCP_Utils::build_meta_permalink( $args );
		...
		public static function meta_archives( $wp_query ) {
			...
			$meta_query = call_user_func( "WPMCP_Search::by_{$meta}", $value );
			...
			foreach ( $defaults as $default ) {
				if ( isset( $vars[ $default ] ) ) {
					...
					if ( $translate || in_array( $default, array( 'meta', 'detail' ) ) )
						$var = WPMCP_L10n::untranslate_rewrite( $var );
					$params[ $default ] = $var;
				}
			}
		...
		public function activate( $network_wide ) {
			...
			delete_option( 'wpmcp_has_custom_pages' );
			if ( ! $this->has_custom_page() )
				add_option( 'wpmcp_has_custom_pages', 'no' );
			else
				add_option( 'wpmcp_has_custom_pages', 'yes' );
		}
