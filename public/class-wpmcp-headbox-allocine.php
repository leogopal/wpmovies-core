...
if ( ! class_exists( 'WPMCP_Headbox_Allocine' ) ) :

	class WPMCP_Headbox_Allocine extends WPMCP_Headbox {
		...
		public function init() {
			...
			$tabs = array(
				...
				'casting' =>  array(
					'title' => __( 'Casting', 'wpmoviescore' ),
					'icon'  => 'actor',
					'content' => $this->render_casting_tab()
				),
				'photos' =>  array(
					'title' => __( 'Photos', 'wpmoviescore' ),
					'icon'  => 'images',
			 ...
			 */
			$this->tabs = apply_filters( 'wpmcp_filter_allocine_headbox_tabs', $tabs );
		...
		public function render( $content = null ) {
			...
			$title = apply_filters( 'wpmcp_format_movie_title', self::get_movie_meta( $id, 'title' ) );
		...
		private function render_main_tab() {
			...
			if ( $images ) {
				...
				$images = WPMoviesCore::render_template( 'shortcodes/images.php', array( 'size' => 'thumbnail', 'movie_id' => $id, 'images' => $images ), $require = 'always' );
			}
			...
			foreach ( $_meta as $m )
				$meta[ $m ] = apply_filters( "wpmcp_format_movie_$m", self::get_movie_meta( $id, $m ) );

			$meta['collections'] = WPMCP_Utils::format_movie_terms_list( $collections, 'collection' );

			$meta['release_date'] = self::get_movie_meta( $id, 'release_date' );
			$meta['year'] = apply_filters( 'wpmcp_format_movie_year', $meta['release_date'], 'Y' );
			$meta['release_date'] = apply_filters( 'wpmcp_format_movie_release_date', $meta['release_date'] );

			$meta = array_map( 'WPMCP_Formatting_Meta::format_movie_field', $meta );

			$meta['cast'] = self::get_movie_meta( $id, 'cast' );
			$casting      = $meta['cast'];
			$meta['cast'] = array_slice( explode( ', ', $meta['cast'] ), 0, 3 );
			$meta['cast'] = implode( ', ', $meta['cast'] );
			$meta['cast'] = apply_filters( 'wpmcp_format_movie_cast', $meta['cast'] );

			$casting = apply_filters( 'wpmcp_format_movie_cast', $casting );
			$casting = array_slice( explode( ', ', $casting ), 0, 4 );

			$rating = apply_filters( 'wpmcp_movie_rating_stars', self::get_movie_meta( $id, 'rating' ), $id, $base = 10 );

			if ( empty( $images ) ) {
				$images = sprintf( __( 'No image to show for %s', 'wpmoviescore' ), '<em>' . $meta['title'] . '</em>' );
			}
		...
		private function render_details_tab() {
			...
			$metas = wpmcp_get_movie_meta();
			$metas = wpmcp_filter_undimension_array( $metas );
				...
				foreach ( $metas as $slug => $field ) {
					...
					if ( isset( $default_fields[ $slug ] ) ) {
						...
						if ( has_filter( "wpmcp_format_movie_{$slug}" ) )
							$field = apply_filters( "wpmcp_format_movie_{$slug}", $field );

						// Filter empty field
						$field = apply_filters( "wpmcp_format_movie_field", $field );

						$metas[ $slug ] = array( 'slug' => $slug, 'title' => __( $default_fields[ $slug ]['title'], 'wpmoviescore' ), 'value' => $field );
					} else {
						unset( $metas[ $slug ] );
					}
				}
		...
		private function render_casting_tab() {
			...
			foreach ( $meta as $slug => $m ) {
				$meta[ $slug ] = apply_filters( "wpmcp_format_movie_$slug", $m );
		...
		private function render_images_tab() {
			...
			if ( $images ) {
				...
				$images = WPMoviesCore::render_template( 'shortcodes/images.php', array( 'size' => 'thumbnail', 'movie_id' => get_the_ID(), 'images' => $images ), $require = 'always' );
			}
			...
			if ( $posters ) {
				...
				$posters = WPMoviesCore::render_template( 'shortcodes/images.php', array( 'size' => 'thumbnail', 'movie_id' => get_the_ID(), 'images' => $posters ), $require = 'always' );
			}

			if ( empty( $images ) ) {
				$images = __( 'No image to show.', 'wpmoviescore' );
			}
			if ( empty( $posters ) ) {
				$posters = __( 'No poster to show.', 'wpmoviescore' );
			}
