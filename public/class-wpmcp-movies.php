<?php
/**
 * WPMoviesCore Movie Class extension.
 * 
...
if ( ! class_exists( 'WPMCP_Movies' ) ) :

	class WPMCP_Movies extends WPMCP_Module {
		...
		public static function register_post_type() {
			...
			$labels = array(
				'name'               => __( 'Movies', 'wpmoviescore' ),
				'singular_name'      => __( 'Movie', 'wpmoviescore' ),
				'add_new'            => __( 'Add New', 'wpmoviescore' ),
				'add_new_item'       => __( 'Add New Movie', 'wpmoviescore' ),
				'edit_item'          => __( 'Edit Movie', 'wpmoviescore' ),
				'new_item'           => __( 'New Movie', 'wpmoviescore' ),
				'all_items'          => __( 'All Movies', 'wpmoviescore' ),
				'view_item'          => __( 'View Movie', 'wpmoviescore' ),
				'search_items'       => __( 'Search Movies', 'wpmoviescore' ),
				'not_found'          => __( 'No movies found', 'wpmoviescore' ),
				'not_found_in_trash' => __( 'No movies found in Trash', 'wpmoviescore' ),
				'parent_item_colon'  => '',
				'menu_name'          => __( 'Movies', 'wpmoviescore' )
			);

			$slug = 'movies';
			if ( '1' == wpmcp_o( 'rewrite-enable' ) ) {
				$rewrite = wpmcp_o( 'rewrite-movie' );
				if ( '' != $slug )
					$slug = $rewrite;
			}
			...
			register_post_status( 'import-queued', array(
				'label'                     => _x( 'Queued Movie', 'wpmoviescore' ),
				'public'                    => false,
				'exclude_from_search'       => true,
				'show_in_admin_all_list'    => false,
				'show_in_admin_status_list' => false,
		...
		public static function get_movie_meta( $post_id = null, $meta = null ) {
			...
			if ( is_admin() && 'data' == $meta && wpmcp_has_deprecated_meta( $post_id ) && wpmcp_o( 'legacy-mode' ) )
				WPMCP_Legacy::update_movie( $post_id );
			...
			if ( 'data' == $meta || 'meta' == $meta ) {
				...
				$value['tmdb_id'] = get_post_meta( $post_id, "_wpmcp_movie_tmdb_id", true );
				$value['poster'] = get_post_meta( $post_id, "_wpmcp_movie_poster", true );

				foreach ( array_keys( $_meta ) as $slug )
					$value[ $slug ] = get_post_meta( $post_id, "_wpmcp_movie_{$slug}", true );
			...
			$value = get_post_meta( $post_id, "_wpmcp_movie_{$meta}", true );
			if ( 'rating' == $meta )
		...
		public static function show_movies_in_home_page( $query ) {
			...
			if ( 1 == wpmcp_o( 'frontpage' ) && is_home() && $query->is_main_query() ) {
		...
		public static function movie_content( $content = null ) {
			...
			if ( wpmcp_o( 'headbox-enable' ) ) {
				$headbox = new WPMCP_Headbox();
		...
		public static function movie_vintage_content( $content = null ) {
			...
			// Naughty PHP 5.3 fix
			$details  = WPMCP_Movies::movie_details();
			$metadata = WPMCP_Movies::movie_metadata();
			 ...
			 */
			$html = apply_filters( 'wpmcp_movie_vintage_content', $html, $details, $metadata, $content );
		...
		public static function movie_excerpt( $excerpt ) {
			...
			$excerpt_length = wpmcp_o( 'excerpt-length' );
			if ( ! $excerpt_length )
		...
		public static function movie_details() {
			...
			if ( 'nowhere' == wpmcp_o( 'show-details' ) || ( 'posts_only' == wpmcp_o( 'show-details' ) && ! is_singular() ) )
				return null;
			...
			foreach ( $fields as $slug => $field ) {
				$detail = call_user_func_array( 'wpmcp_get_movie_meta', array( 'post_id' => $post_id, 'meta' => $slug ) );
				$items[] = apply_filters( "wpmcp_format_movie_{$slug}", $detail );
			}

			$html = WPMoviesCore::render_template( 'movies/movie-details.php', array( 'items' => $items ), $require = 'always' );
		...
		public static function movie_metadata() {
			...
			if ( 'nowhere' == wpmcp_o( 'show-meta' ) || ( 'posts_only' == wpmcp_o( 'show-meta' ) && ! is_singular() ) )
				return null;
			...
			$html = WPMoviesCore::render_template( 'movies/movie-metadata.php', array( 'items' => $items ), $require = 'always' );
		...
		public static function filter_search_query( $wp_query ) {
			...
			$wp_query->set( 'wpmcp_search', $wp_query->get( 's' ) );
			$wp_query->set( 's', null );
		...
		public static function get_search_query( $s ) {
			...
			$search = $wp_query->get( 'wpmcp_search' );
			if( '' != $search );
				$s = $search;
		...
		public static function filter_archives_query( $wp_query ) {
			...
			if ( ( ! is_category() || ( is_category() && '0' == wpmcp_o( 'enable-categories' ) ) ) && ( ! is_tag() || ( is_tag() && '0' == wpmcp_o( 'enable-tags' ) ) ) )
				return $wp_query;
		...
		public static function get_movies( $args = null ) {
			...
			if ( ! is_null( $r['meta_value'] ) ) {
				...
				$meta    = array_keys( WPMCP_Settings::get_supported_movie_meta() );
				$details = array_keys( WPMCP_Settings::get_supported_movie_details() );

				foreach ( $details as $detail )
					if ( ! is_null( $r[ $detail ] ) )
						$meta_query[] = array( 'key' => "_wpmcp_movie_$detail", 'value' => $r['meta_value'], 'compare' => 'LIKE' );

				if ( ! is_null( $r['meta'] ) && in_array( $r['meta'], $meta ) )
					$meta_query[] = array( 'key' => "_wpmcp_movie_{$r['meta']}", 'value' => $r['meta_value'], 'compare' => 'LIKE' );
			}
		...
		public static function clean_movies( $action ) {
			...
			if ( ! $_action || ! isset( $_action[ "wpmcp-{$action}-movies" ] ) )
				return false;

			$action = $_action[ "wpmcp-{$action}-movies" ];
			if ( is_array( $action ) )
				$action = $action[0];
			...
			switch ( $action ) {
				case 'convert':
					foreach ( $contents->posts as $post ) {
						...
						add_post_meta( $post->ID, '_wpmcp_content_type', 'movie', true );
					}
					break;
				case 'remove':
					foreach ( $contents->posts as $post ) {
		...
		public function activate( $network_wide ) {
			...
			$contents = $wpdb->get_results(
				"SELECT DISTINCT post_id
				   FROM {$wpdb->postmeta}
				  WHERE meta_key IN ('_wpml_content_type', '_wpmcp_content_type')
				    AND meta_value='movie'"
			);

			foreach ( $contents as $p ) {
				set_post_type( $p->post_id, 'movie' );
				delete_post_meta( $p->post_id, '_wpmcp_content_type', 'movie' );
