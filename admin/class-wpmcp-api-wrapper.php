<?php
/**
 * WPMoviesCore_Admin Class extension.
 * 
...
if ( ! class_exists( 'WPMCP_TMDb' ) ) :

	class WPMCP_TMDb extends WPMCP_Module {
		...
		public function register_hook_callbacks() {
			...
			add_action( 'wp_ajax_wpmcp_search_movie', __CLASS__ . '::search_movie_callback' );
			add_action( 'wp_ajax_wpmcp_check_api_key', __CLASS__ . '::check_api_key_callback' );
		}
		 ...
		 * to handle the error and add it to the WPMCP_Ajax instance we
		...
		public static function check_api_key_callback() {
			...
			wpmcp_check_ajax_referer( 'check-api-key' );

			if ( ! isset( $_GET['key'] ) || '' == $_GET['key'] || 32 !== strlen( $_GET['key'] ) )
				return new WP_Error( 'invalid', __( 'Invalid API key - the key should be an alphanumerica 32 chars long string.', 'wpmoviescore' ) );

			$check = self::check_api_key( esc_attr( $_GET['key'] ) );

			if ( is_wp_error( $check ) )
				$response = new WP_Error( 'invalid', __( 'Invalid API key - You must be granted a valid key', 'wpmoviescore' ) );
			else
				$response = array( 'message' => __( 'Valid API key - Save your settings and have fun!', 'wpmoviescore' ) );
		...
		public static function search_movie_callback() {
			...
			wpmcp_check_ajax_referer( 'search-movies' );

			$type = ( isset( $_GET['type'] ) && '' != $_GET['type'] ? $_GET['type'] : '' );
			$data = ( isset( $_GET['data'] ) && '' != $_GET['data'] ? $_GET['data'] : '' );
			$lang = ( isset( $_GET['lang'] ) && '' != $_GET['lang'] ? $_GET['lang'] : wpmcp_o( 'api-language' ) );
		...
		private static function get_movie_by_title( $title, $lang, $post_id = null ) {
			...
			$movies = ( wpmcp_o( 'enable-cache' ) ? get_transient( "wpmcp_movie_{$title}_{$lang}" ) : false );

			if ( false === $movies ) {
				$movies = self::_get_movie_by_title( $title, $lang, $post_id );

				if ( true === wpmcp_o( 'enable-cache' ) && ! is_wp_error( $movies ) ) {
					$expire = (int) ( 86400 * wpmcp_o( 'cache-expire' ) );
					set_transient( "wpmcp_movies_{$title}_{$lang}", $movies, $expire );
				}
			}
		...
		public static function _get_movie_by_title( $title, $lang, $post_id = null ) {
			...
			$_result  = 'empty';
			$_message = __( 'Sorry, your search returned no result. Try a more specific query?', 'wpmoviescore' );
			$_movies  = array();
			$_post_id = $post_id;
			...
			if ( isset( $data['status_code'] ) ) {
				...
				$_result  = 'movies';
				$_message = __( 'Your request showed multiple results. Select your movie in the list or try another search:', 'wpmoviescore' );
				$_movies  = array();
				$_post_id = $post_id;
				...
				foreach ( $data['results'] as $movie ) {
					...
					if ( ! is_null( $movie['poster_path'] ) )
						...
						$movie['poster_path'] = str_replace( '{size}', '-medium', WPMCP_DEFAULT_POSTER_URL );

					$_movies[] = array(
						'id'     => $movie['id'],
						'poster' => $movie['poster_path'],
						'title'  => $movie['title'],
						'year'   => apply_filters( 'wpmcp_format_movie_date', $movie['release_date'], 'Y' ),
		...
		public static function get_movie_by_id( $id, $lang, $post_id = null ) {
			...
			$movie = ( wpmcp_o( 'enable-cache' ) ? get_transient( "wpmcp_movie_{$id}_{$lang}" ) : false );

			if ( false === $movie ) {
				$movie = self::_get_movie_by_id( $id, $lang, $post_id );

				if ( true === wpmcp_o( 'enable-cache' ) && ! is_wp_error( $movie ) ) {
					$expire = (int) ( 86400 * wpmcp_o( 'cache-expire' ) );
					set_transient( "wpmcp_movie_{$id}_{$lang}", $movie, 3600 * 24 );
				}
			}
		...
		public static function _get_movie_by_id( $id, $lang, $post_id = null ) {
			...
			$movie = apply_filters( 'wpmcp_filter_meta_data', $movie );
			$casts = apply_filters( 'wpmcp_filter_crew_data', $casts );
			...
			if ( isset( $release['countries'] ) ) {
				...
				foreach ( $release['countries'] as $country ) {
					if ( $country['iso_3166_1'] == wpmcp_o( 'api-country' ) ) {
						$meta['certification'] = $country['certification'];
						$meta['local_release_date'] = $country['release_date'];
					}
					else if ( $country['iso_3166_1'] == wpmcp_o( 'api-country-alt' ) ) {
			...
			if ( is_null( $poster_path ) )
				$poster_path = str_replace( '{size}', '-medium', WPMCP_DEFAULT_POSTER_URL );
			...
			if ( 1 == wpmcp_o( 'actor-autocomplete' ) ) {

				$_movie['taxonomy']['actors'] = array();
				if ( ! empty( $casts['cast'] ) && 1 == wpmcp_o( 'enable-actor' ) ) {
			...
			// Prepare Custom Taxonomy
			if ( 1 == wpmcp_o( 'genre-autocomplete' ) ) {

				$_movie['taxonomy']['genres'] = array();
				if ( ! empty( $movie['genres'] ) && 1 == wpmcp_o( 'enable-genre' ) ) {
		...
		public static function get_movie_images( $tmdb_id ) {
			...
			foreach ( $images as $i => $image ) {
				...
				$exists = apply_filters( 'wpmcp_check_for_existing_images', $tmdb_id, 'image', $file_path );
				if ( false !== $exists )
					unset( $images[ $i ] );
			}
		...
		public static function get_movie_posters( $tmdb_id ) {
			...
			foreach ( $images as $i => $image ) {
				$file_path = substr( $image['file_path'], 1 );
				$exists = apply_filters( 'wpmcp_check_for_existing_images', $tmdb_id, 'poster', $file_path );
				if ( false !== $exists )
					unset( $images[ $i ] );
			}
