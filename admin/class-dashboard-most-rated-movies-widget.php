...
if ( ! class_exists( 'WPMCP_Dashboard_Most_Rated_Movies_Widget' ) ) :

	class WPMCP_Dashboard_Most_Rated_Movies_Widget extends WPMCP_Dashboard {
		 ...
		 * @var      string
		 */
		protected $widget_id = 'wpmcp_dashboard_most_rated_movies_widget';
		...
		private function update_settings() {
			...
			wpmcp_check_admin_referer( "save-{$this->widget_id}" );
			...
			if ( $update ) {
				WPMCP_Utils::admin_notice( __( 'Settings saved.' ), $type = 'update' );
		...
		private function widget_content( $limit = null, $offset = 0 ) {
			...
			foreach ( $movies->posts as $movie ) {
				...
				$movie->meta = array(
					'title'        => apply_filters( 'the_title', wpmcp_get_movie_meta( $movie->ID, 'title' ) ),
					'runtime'      => apply_filters( 'wpmcp_format_movie_runtime', wpmcp_get_movie_meta( $movie->ID, 'runtime' ) ),
					'release_date' => apply_filters( 'wpmcp_format_movie_release_date', wpmcp_get_movie_meta( $movie->ID, 'release_date' ), 'Y' ),
					'overview'     => apply_filters( 'the_content', wpmcp_get_movie_meta( $movie->ID, 'overview' ) )
				);
				$movie->rating  = wpmcp_get_movie_meta( $movie->ID, 'rating' );
				$movie->_rating = apply_filters( 'wpmcp_movie_rating_stars', $movie->rating );
				...
				if ( has_post_thumbnail( $movie->ID ) ) {
					...
					$movie->poster = WPMCP_DEFAULT_POSTER_URL;
		...
		public function dashboard_widget_handle( $context, $object ) {
			...
			if ( $editing && ( ! current_user_can( 'edit_dashboard' ) || ( ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( $_GET['_wpnonce'], "edit_{$this->widget_id}" ) ) ) ) {
				printf( '%s <a href="%s">%s</a>', __( 'You are not allowed to edit this item.' ), admin_url( '/admin.php?page=wpmoviescore' ), __( 'Go back' ) );
				return false;
			}
