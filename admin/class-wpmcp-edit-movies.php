<?php
/**
 * WPMoviesCore Edit_Movies Class extension.
 * 
...
if ( ! class_exists( 'WPMCP_Edit_Movies' ) ) :

	class WPMCP_Edit_Movies extends WPMCP_Module {
		...
		public function register_hook_callbacks() {
			...
			// Post Convert
			if ( 1 == wpmcp_o( 'convert-enable' ) ) {
				add_action( 'admin_footer-edit.php', 'WPMCP_Edit_Movies::bulk_admin_footer', 10 );
				add_action( 'load-post.php', 'WPMCP_Edit_Movies::convert_post_type', 10 );
				add_action( 'load-edit.php', 'WPMCP_Edit_Movies::bulk_convert_post_type', 10 );
			}
			...
			// Callbacks
			add_action( 'wp_ajax_wpmcp_save_meta', __CLASS__ . '::save_meta_callback' );
			add_action( 'wp_ajax_wpmcp_empty_meta', __CLASS__ . '::empty_meta_callback' );
		}
		...
		public function admin_enqueue_scripts( $hook_suffix ) {
			...
			wp_register_script( 'select2-sortable-js', ReduxFramework::$_url . 'assets/js/vendor/select2.sortable.min.js', array( 'jquery' ), WPMCP_VERSION, true );
			wp_register_script( 'select2-js', ReduxFramework::$_url . 'assets/js/vendor/select2/select2.min.js', array( 'jquery', 'select2-sortable-js' ), WPMCP_VERSION, true );
			wp_enqueue_script( 'field-select-js', ReduxFramework::$_url . 'inc/fields/select/field_select.min.js', array( 'jquery', 'select2-js' ), WPMCP_VERSION, true );
			wp_enqueue_style( 'select2-css', ReduxFramework::$_url . 'assets/js/vendor/select2/select2.css', array(), WPMCP_VERSION, 'all' );
			wp_enqueue_style( 'redux-field-select-css', ReduxFramework::$_url . 'inc/fields/select/field_select.css', WPMCP_VERSION, true );
		}
		...
		public static function empty_meta_callback() {
			...
			if ( is_null( $post_id ) )
				return new WP_Error( 'invalid', __( 'Empty or invalid Post ID or Movie Details', 'wpmoviescore' ) );

			wpmcp_check_ajax_referer( 'empty-movie-meta' );

			$response = self::empty_movie_meta( $post_id );

			wpmcp_ajax_response( $response, array(), wpmcp_create_nonce( 'empty-movie-meta' ) );
		}
		...
		public static function save_meta_callback() {
			...
			wpmcp_check_ajax_referer( 'save-movie-meta' );

			$response = self::save_movie_meta( $post_id, $data );

			wpmcp_ajax_response( $response, array(), wpmcp_create_nonce( 'save-movie-meta' ) );
		}
		...
		public static function movie_updated_messages( $messages ) {
			...
			$new_messages = array(
				'movie' => array(
					1  => sprintf( __( 'Movie updated. <a href="%s">View movie</a>', 'wpmoviescore' ), esc_url( get_permalink( $post_ID ) ) ),
					2  => __( 'Custom field updated.', 'wpmoviescore' ) ,
					3  => __( 'Custom field deleted.', 'wpmoviescore' ),
					4  => __( 'Movie updated.', 'wpmoviescore' ),
					5  => isset( $_GET['revision'] ) ? sprintf( __( 'Movie restored to revision from %s', 'wpmoviescore' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
					6  => sprintf( __( 'Movie published. <a href="%s">View movie</a>', 'wpmoviescore' ), esc_url( get_permalink( $post_ID ) ) ),
					7  => __( 'Movie saved.' ),
					8  => sprintf( __( 'Movie submitted. <a target="_blank" href="%s">Preview movie</a>', 'wpmoviescore' ), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ) ),
					9  => sprintf( __( 'Movie scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview movie</a>', 'wpmoviescore' ), date_i18n( __( 'M j, Y @ G:i' ), strtotime( $post->post_date ) ), esc_url( get_permalink( $post_ID ) ) ),
					10 => sprintf( __( 'Movie draft updated. <a target="_blank" href="%s">Preview movie</a>', 'wpmoviescore' ), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ) ),
					11 => __( 'Successfully converted to movie.', 'wpmoviescore' )
				)
			);
		...
		public static function movie_bulk_updated_messages( $bulk_messages, $bulk_counts ) {
			...
			$new_messages = array(
				'movie' => array(
					'updated'   => _n( '%s movie updated.', '%s movies updated.', $bulk_counts['updated'], 'wpmoviescore' ),
					'locked'    => _n( '%s movie not updated, somebody is editing it.', '%s movies not updated, somebody is editing them.', $bulk_counts['locked'], 'wpmoviescore' ),
					'deleted'   => _n( '%s movie permanently deleted.', '%s movies permanently deleted.', $bulk_counts['deleted'], 'wpmoviescore' ),
					'trashed'   => _n( '%s movie moved to the Trash.', '%s movies moved to the Trash.', $bulk_counts['trashed'], 'wpmoviescore' ),
					'untrashed' => _n( '%s movie restored from the Trash.', '%s movies restored from the Trash.', $bulk_counts['untrashed'], 'wpmoviescore' ),
				)
			);
		...
		public static function bulk_admin_footer() {
			...
			$post_types = wpmcp_o( 'convert-post-types' );
			if ( ! in_array( $post_type, $post_types ) )
				return false;
?>
	<script type="text/javascript">
		(function( $ ) {
			$( '<option>' ).val( 'bulk_convert_post_type' ).text( '<?php _e( 'Convert to movie', 'wpmoviescore' )?>' ).appendTo( "select[name='action']" );
			$( '<option>' ).val( 'bulk_convert_post_type' ).text( '<?php _e( 'Convert to movie', 'wpmoviescore' )?>' ).appendTo( "select[name='action2']" );
		})(jQuery);
	</script>
...
<?php
		...
		public static function convert_post_type() {
			...
			if ( ! isset( $_REQUEST['wpmcp_convert_post_type'] ) || '1' != $_REQUEST['wpmcp_convert_post_type'] )
				return false;

			wpmcp_check_admin_referer( 'convert-post-type' );
		...
		public static function bulk_convert_posts( $post_ids ) {
			...
			foreach ( $post_ids as $post_id ) {
				...
				if ( ! $update )
					$error->add( $post_id, sprintf( __( 'Error update Post #%d post_type.', 'wpmoviescore' ), $post_id ) );
			}
		...
		public static function movies_columns_head( $defaults ) {
			...
			$defaults = array_merge(
				...
				array( 'wpmcp-poster' => __( 'Poster', 'wpmoviescore' ) ),
				array_slice( $defaults, $title, $comments, true ),
				array( 'wpmcp-release_date' => sprintf( '<span class="wpmolicon icon-date" title="%s"></span>', __( 'Year', 'wpmoviescore' ) ) ),
				array( 'wpmcp-status'       => sprintf( '<span class="wpmolicon icon-status" title="%s"></span>', __( 'Status', 'wpmoviescore' ) ) ),
				array( 'wpmcp-media'        => sprintf( '<span class="wpmolicon icon-video" title="%s"></span>', __( 'Media', 'wpmoviescore' ) ) ),
				array( 'wpmcp-rating'       => __( 'Rating', 'wpmoviescore' ) ),
		...
		public static function movies_columns_content( $column_name, $post_id ) {
			...
			$_column_name = str_replace( 'wpmcp-', '', $column_name );
			switch ( $column_name ) {
				case 'wpmcp-poster':
					$html = get_the_post_thumbnail( $post_id, 'thumbnail' );
					break;
				case 'wpmcp-release_date':
					$meta = wpmcp_get_movie_meta( $post_id, 'release_date' );
					$html = apply_filters( 'wpmcp_format_movie_release_date', $meta, 'Y' );
					break;
				case 'wpmcp-status':
					$meta = call_user_func_array( 'wpmcp_get_movie_meta', array( 'post_id' => $post_id, 'meta' => $_column_name ) );
					$html = apply_filters( 'wpmcp_format_movie_status', $meta, $format = 'html', $icon = true );
					break;
				case 'wpmcp-media':
					$meta = call_user_func_array( 'wpmcp_get_movie_meta', array( 'post_id' => $post_id, 'meta' => $_column_name ) );
					$html = apply_filters( 'wpmcp_format_movie_media', $meta, $format = 'html', $icon = true );
					break;
				case 'wpmcp-rating':
					$meta = wpmcp_get_movie_rating( $post_id );
					$html = apply_filters( 'wpmcp_movie_rating_stars', $meta, $post_id, $base = 5 );
					break;
				default:
					$html = '';
					break;
		...
		public static function movies_sortable_columns( $columns ) {
			...
			$columns['wpmcp-release_date'] = 'wpmcp-release_date';
			$columns['wpmcp-status']       = 'wpmcp-status';
			$columns['wpmcp-media']        = 'wpmcp-media';
			$columns['wpmcp-rating']       = 'wpmcp-rating';
		...
		public static function movies_sortable_columns_order( $wp_query ) {
			...
			$allowed = array( 'wpmcp-release_date', 'wpmcp-release_date', 'wpmcp-status', 'wpmcp-media', 'wpmcp-rating' );
			if ( in_array( $orderby, $allowed ) ) {
				$key = str_replace( 'wpmcp-', '_wpmcp_movie_', $orderby );
		...
		public static function quick_edit_movies( $column_name, $post_type ) {
			...
			if ( 'movie' != $post_type || 'wpmcp-poster' != $column_name || 1 !== did_action( 'quick_edit_custom_box' ) )
				return false;
		...
		public static function bulk_edit_movies( $column_name, $post_type ) {
			...
			if ( 'movie' != $post_type || 'wpmcp-poster' != $column_name || 1 !== did_action( 'bulk_edit_custom_box' ) )
				return false;
		...
		public static function expand_quick_edit_link( $actions, $post ) {
			...
			$nonce    = wpmcp_create_nonce( 'set-quickedit-movie-details' );
			$details  = WPMCP_Settings::get_supported_movie_details();
			$_details = array_keys( $details );

			foreach ( $_details as $i => $detail ) {
				$data = call_user_func_array( 'wpmcp_get_movie_meta', array( 'post_id' => $post->ID, 'meta' => $detail ) );
			...
			$actions['inline hide-if-no-js'] .= " onclick=\"wpmcp_edit_movies.quick_edit({$_details}, '{$nonce}')\">"; 
		...
		public static function load_movie_images( $tmdb_id, $post ) {
			...
			$images = WPMCP_TMDb::get_movie_images( $tmdb_id );
			$images = apply_filters( 'wpmcp_jsonify_movie_images', $images, $post, 'image' );
		...
		public static function load_movie_posters( $tmdb_id, $post ) {
			...
			$posters = WPMCP_TMDb::get_movie_posters( $tmdb_id );
			$posters = apply_filters( 'wpmcp_jsonify_movie_images', $posters, $post, 'poster' );
		...
		public static function movie_metabox( $post, $args = array() ) {
			 ...
			 * @since    2.0
			 *
			 * @param    array    $wpmcp_metabox_panels Existing Panels
			 */
			$args['panels'] = apply_filters( 'wpmcp_filter_metabox_panels', $args['panels'] );
		...
		private static function render_preview_panel( $post_id ) {
			...
			$rating   = wpmcp_get_movie_rating( $post_id );
			$metadata = wpmcp_get_movie_meta( $post_id );
			$metadata = wpmcp_filter_empty_array( $metadata );
			...
			if ( $empty )
				...
				foreach ( $metadata as $slug => $meta )
					$preview[ $slug ] = call_user_func( 'apply_filters', "wpmcp_format_movie_{$slug}", $meta );

			$attributes = array(
				'empty'     => $empty,
				'thumbnail' => get_the_post_thumbnail( $post_id, 'medium' ),
				'rating'    => apply_filters( 'wpmcp_movie_rating_stars', $rating, $post_id, $base = 5 ),
				'preview'   => $preview
			);
		...
		private static function render_meta_panel( $post_id ) {
			...
			$metas     = WPMCP_Settings::get_supported_movie_meta();
			$languages = WPMCP_Settings::get_supported_languages();
			$metadata  = wpmcp_get_movie_meta( $post_id );
			$metadata  = wpmcp_filter_empty_array( $metadata );
		...
		private static function render_details_panel( $post_id ) {
			...
			foreach ( $details as $slug => $detail ) {
				...
				$value      = call_user_func_array( 'wpmcp_get_movie_meta', array( 'post_id' => $post_id, 'meta' => $slug ) );

				if ( ! class_exists( $class_name ) )
					require_once WPMCP_PATH . "includes/framework/redux/ReduxCore/inc/fields/{$field_name}/field_{$field_name}.php";
		...
		private static function render_images_panel( $post_id ) {
			...
			$attributes = array(
				'nonce'   => wpmcp_nonce_field( 'upload-movie-image', $referer = false ),
				'images'  => WPMCP_Media::get_movie_imported_images(),
		...
		private static function render_posters_panel( $post_id ) {
			...
			$attributes = array(
				'posters' => WPMCP_Media::get_movie_imported_posters(),
				'version' => ( version_compare( $wp_version, '4.0', '>=' ) ? 4 : 0 )
			);
		...
		public static function save_movie_details( $post_id, $details ) {
			...
			if ( ! $post || 'movie' != get_post_type( $post ) )
				return new WP_Error( 'invalid_post', __( 'Error: submitted post is not a movie.', 'wpmoviescore' ) );

			$details    = self::validate_details( $details );
			$supported  = WPMCP_Settings::get_supported_movie_details();

			if ( ! is_array( $details ) )
				return new WP_Error( 'invalid_details', __( 'Error: the submitted movie details are invalid.', 'wpmoviescore' ) );

			foreach ( $details as $slug => $detail )
				update_post_meta( $post_id, "_wpmcp_movie_{$slug}", $detail );

			WPMCP_Cache::clean_transient( 'clean', $force = true );
		...
		public static function save_movie_meta( $post_id, $movie_meta, $clean = true ) {
			...
			foreach ( $movie_meta as $slug => $meta )
				$update = update_post_meta( $post_id, "_wpmcp_movie_{$slug}", $meta );
		...
		private static function validate_meta( $data ) {
			...
			$data = wpmcp_filter_empty_array( $data );
			$data = wpmcp_filter_undimension_array( $data );

			$supported = WPMCP_Settings::get_supported_movie_meta();
		...
		public static function empty_movie_meta( $post_id ) {
			...
			delete_post_meta( $post_id, '_wpmcp_movie_data' );
		...
		public static function save_movie( $post_ID, $post, $queue = false, $movie_meta = null ) {
			...
			if ( ! current_user_can( 'edit_post', $post_ID ) )
				return new WP_Error( __( 'You are not allowed to edit posts.', 'wpmoviescore' ) );

			if ( ! $post = get_post( $post_ID ) || 'movie' != get_post_type( $post ) )
				return new WP_Error( sprintf( __( 'Posts with #%s is invalid or is not a movie.', 'wpmoviescore' ), $post_ID ) );
			...
			if ( ! is_null( $movie_meta ) && count( $movie_meta ) ) {
				...
				if ( wpmcp_o( 'poster-featured' ) && ! $queue ) {
					$upload = WPMCP_Media::set_image_as_featured( $movie_meta['poster'], $post_ID, $movie_meta['tmdb_id'], $movie_meta['title'] );
				...
				// Autofilling Actors
				if ( wpmcp_o( 'enable-actor' ) && wpmcp_o( 'actor-autocomplete' ) ) {
					$limit = intval( wpmcp_o( 'actor-limit' ) );
				...
				// Autofilling Genres
				if ( wpmcp_o( 'enable-genre' ) && wpmcp_o( 'genre-autocomplete' ) ) {
					$genres = explode( ',', $movie_meta['genres'] );
					$genres = wp_set_object_terms( $post_ID, $genres, 'genre', false );
				}

				// Autofilling Collections
				if ( wpmcp_o( 'enable-collection' ) && wpmcp_o( 'collection-autocomplete' ) ) {
			...
			if ( isset( $_REQUEST['wpmcp_details'] ) && ! is_null( $_REQUEST['wpmcp_details'] ) ) {

				if ( isset( $_REQUEST['is_quickedit'] ) || isset( $_REQUEST['is_bulkedit'] ) )
					wpmcp_check_admin_referer( 'quickedit-movie-details' );

				$wpmcp_details = $_REQUEST['wpmcp_details'];
				if ( true === $_REQUEST['is_bulkedit'] ) {
					foreach ( $_REQUEST['post'] as $post_id ) {
						self::save_movie_details( $post_id, $wpmcp_details );
					}
				} else {
					self::save_movie_details( $post_ID, $wpmcp_details );
				}
