<?php
/**
 * WPMoviesCore Import Class extension.
 * 
 * Import Movies
 *
...
if ( ! class_exists( 'WPMCP_Import' ) ) :

	class WPMCP_Import extends WPMCP_Module {
		...
		public function register_hook_callbacks() {
			...
			add_action( 'wp_ajax_wpmcp_delete_movies', __CLASS__ . '::delete_movies_callback' );
			add_action( 'wp_ajax_wpmcp_import_movies', __CLASS__ . '::import_movies_callback' );
			add_action( 'wp_ajax_wpmcp_imported_movies', __CLASS__ . '::imported_movies_callback' );
		}
		...
		public static function delete_movies_callback() {
			...
			wpmcp_check_ajax_referer( 'delete-movies' );

			$movies = ( isset( $_GET['movies'] ) && '' != $_GET['movies'] ? $_GET['movies'] : null );

			$response = self::delete_movies( $movies );
			wpmcp_ajax_response( $response, array(), wpmcp_create_nonce( 'delete-movies' ) );
		}

		/**
		 * Callback for Imported Movies WPMCP_Import_Table AJAX navigation.
		 * 
		 * Checks the AJAX nonce, create a new instance of WPMCP_Import_Table
		...
		public static function imported_movies_callback() {
			...
			wpmcp_check_ajax_referer( 'imported-movies' );

			$wp_list_table = new WPMCP_Import_Table();
			$wp_list_table->ajax_response();
		}

		/**
		 * Callback for WPMCP_Import movie import method.
		 * 
		...
		public static function import_movies_callback() {
			...
			wpmcp_check_ajax_referer( 'import-movies-list' );

			$movies = ( isset( $_POST['movies'] ) && '' != $_POST['movies'] ? esc_textarea( $_POST['movies'] ) : null );

			$response = self::import_movies( $movies );
			wpmcp_ajax_response( $response, array(), wpmcp_create_nonce( 'import-movies-list' ) );
		}
...
<?php
		...
		public static function delete_movies( $movies ) {
			...
			if ( is_null( $movies ) || ! is_array( $movies ) ) {
				$errors->add( 'invalid', __( 'Invalid movie list submitted.', 'wpmoviescore' ) );
				return $errors;
			}

			$response = wpmcp_ajax_filter( array( __CLASS__, 'delete_movie' ), array( $movies ), $loop = true );
		...
		public static function delete_movie( $post_id ) {
			...
			if ( false === wp_delete_post( $post_id, $force_delete = true ) )
				return new WP_Error( 'error', sprintf( __( 'An error occured trying to delete Post #%s', 'wpmoviescore' ), $post_id ) );

			$thumb_id = get_post_thumbnail_id( $post_id );
			if ( '' != $thumb_id )
				if ( false === wp_delete_attachment( $thumb_id ) )
					return new WP_Error( 'error', sprintf( __( 'An error occured trying to delete Attachment #%s', 'wpmoviescore' ), $thumb_id ) );
		...
		private static function import_movies( $movies ) {
			...
			$response = wpmcp_ajax_filter( array( __CLASS__, 'import_movie' ), array( $movies ), $loop = true );
		...
		public static function import_movie( $movie ) {
			...
			if ( ! is_null( $page ) ) {
				$message = sprintf( '%s − <span class="edit"><a href="%s">%s</a> |</span> <span class="view"><a href="%s">%s</a></span>',
					sprintf( __( 'Movie "%s" already imported.', 'wpmoviescore' ), "<em>" . get_the_title( $page->ID ) . "</em>" ),
					get_edit_post_link( $page->ID ),
					__( 'Edit', 'wpmoviescore' ),
					get_permalink( $page->ID ),
					__( 'View', 'wpmoviescore' )
				);
				...
				while ( have_posts() ) {
					...
					if ( 'import-draft' == get_post_status() ) {
						$columns[ get_the_ID() ] = array(
							...
							'director'   => get_post_meta( get_the_ID(), '_wpmcp_tmdb_director', true ),
							'tmdb_id'    => get_post_meta( get_the_ID(), '_wpmcp_tmdb_id', true )
						);
					}
				}
			}
		...
		public static function import_movie_list_add_options() {
			...
			$option = 'per_page';
			$args = array(
				'label'   => __( 'Import Drafts', 'wpmoviescore' ),
				'default' => 30,
		...
		public static function import_page() {
			...
			if ( isset( $_POST['wpmcp_save_imported'] ) && '' != $_POST['wpmcp_save_imported'] && isset( $_POST['wpmcp_imported_ids'] ) && '' != $_POST['wpmcp_imported_ids'] && isset( $_POST['movies'] ) && count( $_POST['movies'] ) ) {

				wpmcp_check_admin_referer( 'save-imported-movies' );

				$post_ids = explode( ',', $_POST['wpmcp_imported_ids'] );

				foreach ( $_POST['movies'] as $movie ) {
					if ( 0 != $movie['tmdb_id'] && in_array( $movie['post_id'], $post_ids ) ) {
						$update = WPMCP_Edit_Movies::save_movie( $movie['post_id'], $post = null, $queue = false, $movie );
				...
				if ( ! empty( $errors->errors ) ) {
					...
					WPMCP_Utils::admin_notice( sprintf( __( 'The following errors occured: <ul>%s</ul>', 'wpmoviescore' ), implode( '', $_errors ) ), 'error' );
				}

				if ( ! empty( $imported ) )
					WPMCP_Utils::admin_notice( sprintf( _n( 'One movie imported successfully!', '%d movies imported successfully!', count( $imported ), 'wpmoviescore' ), count( $imported ) ), 'updated' );
			}
			else if ( isset( $_POST['wpmcp_importer'] ) && '' != $_POST['wpmcp_importer'] ) {


			}

			if ( isset( $_GET['wpmcp_section'] ) && in_array( $_GET['wpmcp_section'], array( 'wpmcp_import', 'wpmcp_import_queue', 'wpmcp_imported' ) ) )
				$_section =  $_GET['wpmcp_section'];
