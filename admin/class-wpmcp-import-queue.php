<?php
/**
 * WPMoviesCore Queue Class extension.
 * 
 * Queued Movies
 *
...
if ( ! class_exists( 'WPMCP_Queue' ) ) :

	class WPMCP_Queue extends WPMCP_Module {
		...
		public function register_hook_callbacks() {
			...
			add_action( 'wp_ajax_wpmcp_enqueue_movies', __CLASS__ . '::enqueue_movies_callback' );
			add_action( 'wp_ajax_wpmcp_dequeue_movies', __CLASS__ . '::dequeue_movies_callback' );
			add_action( 'wp_ajax_wpmcp_queued_movies', __CLASS__ . '::queued_movies_callback' );
			add_action( 'wp_ajax_wpmcp_import_queued_movie', __CLASS__ . '::import_queued_movie_callback' );
		}

		/**
		 * Callback for WPMCP_Queue movie enqueue method.
		 * 
		...
		public static function enqueue_movies_callback() {
			...
			wpmcp_check_ajax_referer( 'enqueue-movies' );

			$movies = ( isset( $_POST['movies'] ) && '' != $_POST['movies'] ? $_POST['movies'] : null );

			$response = self::enqueue_movies( $movies );
			wpmcp_ajax_response( $response, array(), wpmcp_create_nonce( 'enqueue-movies' ) );
		}

		/**
		 * Callback for WPMCP_Queue movie dequeue method.
		 * 
		...
		public static function dequeue_movies_callback() {
			...
			wpmcp_check_ajax_referer( 'dequeue-movies' );

			$movies = ( isset( $_POST['movies'] ) && '' != $_POST['movies'] ? $_POST['movies'] : null );

			$response = self::dequeue_movies( $movies );
			wpmcp_ajax_response( $response, array(), wpmcp_create_nonce( 'dequeue-movies' ) );
		}
		...
		public static function queued_movies_callback() {
			...
			wpmcp_check_ajax_referer( 'queued-movies' );
			...
			wpmcp_ajax_response( $response, $i18n, wpmcp_create_nonce( 'queued-movies' ) );
		}
		...
		public static function import_queued_movie_callback() {
			...
			wpmcp_check_ajax_referer( 'import-queued-movies' );

			$post_id = ( isset( $_POST['post_id'] ) && '' != $_POST['post_id'] ? $_POST['post_id'] : null );

			$response = self::import_queued_movie( $post_id );
			wpmcp_ajax_response( $response, array(), wpmcp_create_nonce( 'import-queued-movies' ) );
		}
		...
		private static function enqueue_movies( $movies ) {
			...
			$response = wpmcp_ajax_filter( array( __CLASS__, 'enqueue_movie' ), array( $movies ), $loop = true );
			return $response;
		}
		...
		public static function enqueue_movie( $movie ) {
			...
			if ( is_wp_error( $update ) )
				return new WP_Error( 'error', sprintf( __( 'An error occured when adding "%s" to the queue: %s', 'wpmoviescore' ), $post_title, $update->get_error_message() ) );

			$update = WPMCP_Edit_Movies::save_movie( $update, $post = null, $queue = true, $metadata );
			if ( is_wp_error( $update ) )
				return new WP_Error( 'error', sprintf( __( 'An error occured when adding "%s" to the queue: %s', 'wpmoviescore' ), $post_title, $update->get_error_message() ) );
		...
		private static function dequeue_movies( $movies ) {
			...
			$response = wpmcp_ajax_filter( array( __CLASS__, 'dequeue_movie' ), array( $movies ), $loop = true );
			return $response;
		}
		...
		public static function dequeue_movie( $post_id ) {
			...
			$update = wp_update_post( $_post, $wp_error = true );
			if ( is_wp_error( $update ) )
				return new WP_Error( 'error', sprintf( __( 'An error occured when trying to remove "%s" from the queue: %s', 'wpmoviescore' ), get_the_title( $post_id ), $update->get_error_message() ) );

			$update = delete_post_meta( $post_id, '_wpmcp_movie_data' );
			if ( false === $update )
				return new WP_Error( 'error', sprintf( __( 'An error occured when trying to delete "%s" metadata.', 'wpmoviescore' ), get_the_title( $post_id ) ) );
		...
		private static function import_queued_movie( $post_id ) {
			...
			if ( is_null( $post_id ) || ! $post = get_post( $post_id ) || 'movie' != get_post_type( $post_id ) )
				return new WP_Error( 'invalid_movie', sprintf( __( 'Error: submitted Post ID doesn\t match any valid movie.', 'wpmoviescore' ) ) );

			$meta = wpmcp_get_movie_meta( $post_id );
			if ( '' == $meta || ! is_array( $meta ) || ! isset( $meta['poster'] ) || ! isset( $meta['tmdb_id'] ) )
				return new WP_Error( 'invalid_meta', sprintf( __( 'Error: cannot find submitted movie\'s metadata, try enqueuing it again.', 'wpmoviescore' ) ) );
			...
			if ( wpmcp_o( 'poster-featured' ) ) {
				$id = WPMCP_Media::set_image_as_featured( $meta['poster'], $post_id, $meta['tmdb_id'], $meta['title'] );
				update_post_meta( $post_id, '_thumbnail_id', $id );
			}

			$update = wp_update_post( $_post, $wp_error = true );
			if ( is_wp_error( $update ) )
				return new WP_Error( 'error', sprintf( __( 'An error occured when trying to imported queued movie "%s": %s', 'wpmoviescore' ), get_the_title( $post_id ), $update->get_error_message() ) );
				...
				while ( have_posts() ) {
					...
					if ( 'import-queued' == get_post_status() ) {
						$columns[ get_the_ID() ] = array(
							'ID'       => get_the_ID(),
							'title'    => get_the_title(),
							'director' => wpmcp_get_movie_meta( get_the_ID(), 'director' ),
							'tmdb_id'  => wpmcp_get_movie_meta( get_the_ID(), 'tmdb_id', true )
						);
					}
				}
			}
