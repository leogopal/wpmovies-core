...
if ( ! class_exists( 'TMDb' ) ) :
	...
	class TMDb
	{
		...
		public function __construct( $config = false, $dummy = false ) {
			...
			$this->api_key  = wpmcp_o( 'api-key' );
			$this->scheme   = wpmcp_o( 'api-scheme' );
			$this->internal = wpmcp_o( 'api-internal' );
		...
		public function searchMovie( $query, $page = 1, $adult = false, $year = null, $lang = null ) {
			...
			$params = array(
				'query'         => $query,
				'page'          => (int) $page,
				'language'      => is_null( $lang ) ? wpmcp_o( 'api-language' ) : $lang,
		...
		public function getMovie( $id, $lang = null ) {
			...
			$params = array( 'language' => is_null( $lang ) ? wpmcp_o( 'api-language' ) : $lang );
		...
		public function getConfiguration() {
			...
			$config = WPMCP_Cache::get( 'tmdb_api_config' );
			if ( ! $config ) {
				...
				if ( is_wp_error( $config ) ) {
					...
					WPMCP_Utils::admin_notice( $config->get_error_message(), 'error' );
					return array();
				}
				...
				WPMCP_Cache::set( 'tmdb_api_config', $config );
			}
		...
		public function getImageUrl( $filepath, $imagetype, $size ) {
			...
			if ( isset( $config['images'] ) ) {
				...
				if ( isset( $config['images'][ $imagetype . '_sizes' ][ $_size ] ) )
					...
					return new WP_Error( 'wrong_size', sprintf( __( 'The size "%s" is not supported by TMDb', 'wpmoviescore' ), $size ) );
			}
			else
				return new WP_Error( 'no_config', __( 'No configuration available for image URL generation', 'wpmoviescore' ) );
		}
		...
		public function getAvailableImageSizes( $imagetype ) {
			...
			if ( isset( $config['images'][$imagetype.'_sizes'] ) )
				...
				return new WP_Error( 'no_config', __( 'API Error: no configuration available to retrieve available image sizes', 'wpmoviescore' ) );
		}
		...
		protected function _makeCall( $function, $params = null, $session_id = null, $method = 'get' ) {
			...
			if ( isset( $response['response']['code'] ) && 200 != $response['response']['code'] )
				return new WP_Error( 'connect_failed', sprintf( __( 'API Error: server connection to "%s" returned error %s: %s', 'wpmoviescore' ), $url, $response['response']['code'], $response['response']['message'] ) );
			...
			if ( is_array( $body ) && array_key_exists( 'status_code', $body ) && array_key_exists( 'status_message', $body ) )
				return new WP_Error( 'connect_failed', sprintf( __( 'API Error: connection to TheMovieDB API failed with message "%s" (code %s)', 'wpmoviescore' ), $body['status_code'], $body['status_message'] ) );

			if ( is_null( $results ) )
				return new WP_Error( 'unknown_error', __( 'API Error: unknown server error, unable to perform request.', 'wpmoviescore' ) );
