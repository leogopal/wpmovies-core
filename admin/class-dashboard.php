...
if ( ! class_exists( 'WPMCP_Dashboard' ) ) :

	class WPMCP_Dashboard extends WPMCP_Module {
		...
		public function init() {
			...
			global $wpmcp_dashboard_widgets;

			$this->widgets = array();
			$this->allowed_options = array( 'welcome_panel' );

			foreach ( $wpmcp_dashboard_widgets as $slug => $widget ) {
		...
		public function register_hook_callbacks() {
			...
			add_filter( 'wpmcp_filter_widget_classname', array( $this, 'filter_widget_classname' ), 10, 1 );

			add_action( 'wp_ajax_wpmcp_save_screen_option', array( $this, 'wpmcp_save_screen_option_callback' ) );
			add_action( 'wp_ajax_wpmcp_save_dashboard_widget_settings', __CLASS__ . '::wpmcp_save_dashboard_widget_settings_callback' );
			add_action( 'wp_ajax_wpmcp_load_more_movies', __CLASS__ . '::wpmcp_load_more_movies_callback' );
		}
		 ...
		 * @since    1.0
		 */
		public function wpmcp_save_screen_option_callback() {
		 ...
		 * @since    1.0
		 */
		public static function wpmcp_save_dashboard_widget_settings_callback() {
			...
			wpmcp_check_ajax_referer( 'save-' . strtolower( $widget ) );

			$class = $widget::get_instance();
			$update = self::save_widget_setting( $class->widget_id, $setting, $value );

			WPMCP_Utils::ajax_response( $update, array(), WPMCP_Utils::create_nonce( 'save-' . strtolower( $widget ) ) );
		}
		 ...
		 * @since    1.0
		 */
		public static function wpmcp_load_more_movies_callback() {

			wpmcp_check_ajax_referer( 'load-more-widget-movies' );
		...
		public function screen_options( $status, $args ) {
			...
			if ( $args->base != 'toplevel_page_wpmoviescore' )
				return $status;
			...
			$return = array( '<h5>' . __( 'Show on screen', 'wpmoviescore' ) . '</h5>' );
			$return[] = $this->set_screen_option( 'welcome_panel', __( 'Welcome', 'wpmoviescore' ), $status );

			global $wpmcp_dashboard_widgets;
			foreach ( $wpmcp_dashboard_widgets as $slug => $widget )
				$return[] = $this->set_screen_option( $slug, $widget['title'], $status );

			$return[] = get_submit_button( __( 'Apply', 'wpmoviescore' ), 'button hide-if-js', 'screen-options-apply', false );
		...
		private function set_screen_option( $option, $title, $status ) {
			...
			$visible = ( in_array( 'wpmcp_dashboard_' . $option . '_widget', $hidden ) ? '0' : '1' );

			$return = $status . '<label for="show_wpmcp_' . $option . '"><input id="show_wpmcp_' . $option . '" type="checkbox"' . checked( $visible, '1', false ) . ' />' . __( $title, 'wpmoviescore' ) . '</label>';
		...
		private static function save_screen_option( $option, $value, $screen_id ) {
			...
			$option = 'wpmcp_dashboard_' . $option . '_widget';
		...
		public static function dashboard() {
			...
			if ( isset( $_GET['hide_wpmcp_api_key_notice'] ) && ( isset( $_GET['_nonce'] ) || ! wp_verify_nonce( $_GET['_nonce'], 'hide-wpmcp-api-key-notice' ) ) )
				WPMoviesCore_Admin::show_api_key_notice();

			if ( isset( $_GET['wpmcp_set_archive_page'] ) && ( isset( $_GET['_nonce'] ) || ! wp_verify_nonce( $_GET['_nonce'], 'wpmcp-set-archive-page' ) ) )
				WPMCP_Utils::set_archive_page();

			global $wpmcp_dashboard_widgets;
			foreach ( $wpmcp_dashboard_widgets as $widget )
		...
		public static function add_dashboard_widget( $widget ) {
			...
			$class = apply_filters( 'wpmcp_filter_widget_classname', $class );
			$widget_id = strtolower( $class );
			$widget_name = __( $name, 'wpmoviescore' );
			...
			if ( ! is_null( $control_callback ) && current_user_can( 'edit_dashboard' ) && is_callable( $control_callback ) ) {
				...
				$widget_name = __( $widget_name, 'wpmoviescore' );
		...
		public function filter_widget_classname( $name ) {
			...
			return "WPMCP_Dashboard_{$name}_Widget";
		}
