...
class WPMCP_Import_Table extends WPMCP_List_Table {
	...
	function __construct() {
		...
		$this->metadata = WPMCP_Settings::get_supported_movie_meta();

		$this->columns = WPMCP_Import::get_imported_movies();

		$this->column_names = array(
			'poster'     => __( 'Poster', 'wpmoviescore' ),
			'movietitle' => __( 'Title', 'wpmoviescore' ),
			'director'   => __( 'Director', 'wpmoviescore' ),
			'actions'    => __( 'Actions', 'wpmoviescore' )
		);
	}
	 ...
	 */
	function no_items() {
		_e( 'No movies found, dude.', 'wpmoviescore' );
	}
	...
	function pagination( $which ) {
		...
		$page_links[] = sprintf( "<a class='%s' title='%s' href='%s' onclick='wpmcp_import_view.navigate( this ); return false;'>%s</a>",
		...
		if ( 'bottom' == $which )
			...
			$html_current_page = sprintf( "<input class='current-page' title='%s' type='text' name='paged' value='%s' size='%d' oninput='wpmcp_import_view.paginate(); return false;' />",
	...
	function column_actions( $item ) {
		...
		$actions = array(
			'edit'      => sprintf('<a class="edit_movie" id="edit_%1$s" href="%2$s" title="%3$s"><span class="wpmolicon icon-edit-page"></span></a>', $item['ID'], get_edit_post_link( $item['ID'] ), __( 'Edit', 'wpmoviescore' ) ),
			'metadata'  => sprintf('<a class="search_movie" id="search_%1$s" href="%2$s" title="%3$s" onclick="wpmcp_import_meta.search(%4$s); return false;"><span class="wpmolicon icon-import"></span></a>', $item['ID'], wp_nonce_url( get_edit_post_link( $item['ID'] ) . "&amp;wpmcp_search_movie=1&amp;search_by=title&amp;search_query={$item['movietitle']}", 'search-movies' ), __( 'Fetch data from TMDb', 'wpmoviescore' ), $item['ID'] ),
			'enqueue hide-if-no-js'   => sprintf('<a class="enqueue_movie" id="enqueue_%1$s" href="#" title="%2$s" onclick="wpmcp_movies_queue.add(%3$s); return false;"><span class="wpmolicon icon-plus"></span></a>', $item['ID'], __( 'Enqueue', 'wpmoviescore' ), $item['ID'] ),
			'delete'    => sprintf('<a class="delete_movie" id="delete_%1$s" href="%2$s" title="%3$s" onclick="wpmcp_import_movies.delete(%4$s); return false;"><span class="wpmolicon icon-trash"></span></a>', $item['ID'], get_delete_post_link( $item['ID'] ), __( 'Delete', 'wpmoviescore' ), $item['ID'] ),
		);
	...
	function get_bulk_actions() {
		...
		$actions = array(
			'search'  => __( 'Find Metadata', 'wpmoviescore' ),
			'enqueue' => __( 'Enqueue Movie', 'wpmoviescore' ),
			'delete'  => __( 'Delete Movie', 'wpmoviescore' ),
		);
	...
	function bulk_actions( $which = '' ) {
		...
		submit_button( __( 'Apply' ), 'action', false, false, array( 'id' => "doaction$two", 'onclick' => "wpmcp_import_meta.do( 'action$two' ); return false;" ) );
		echo "\n";
	}
	 ...
	 * @uses WPMCP_List_Table::get_columns()
	 * @uses WPMCP_List_Table::get_sortable_columns()
	 * @uses WPMCP_List_Table::get_items_per_page()
	 * @uses WPMCP_List_Table::filter_search()
	 *
	 * @since 1.0
	 */
	...
	function ajax_response() {
		...
		wpmcp_ajax_response( $response, $i18n );
	}
