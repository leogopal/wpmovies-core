<?php
/**
 * WPMoviesCore Media Class extension.
 * 
...
if ( ! class_exists( 'WPMCP_Media' ) ) :

	class WPMCP_Media extends WPMCP_Module {
		...
		public function register_hook_callbacks() {
			...
			add_filter( 'wpmcp_check_for_existing_images', __CLASS__ . '::check_for_existing_images', 10, 3 );
			add_filter( 'wpmcp_jsonify_movie_images', __CLASS__ . '::fake_jsonify_movie_images', 10, 3 );
			add_action( 'wp_ajax_wpmcp_upload_image', __CLASS__ . '::upload_image_callback' );
			add_action( 'wp_ajax_wpmcp_set_featured', __CLASS__ . '::set_featured_image_callback' );
		}
		...
		public static function upload_image_callback() {
			...
			wpmcp_check_ajax_referer( 'upload-movie-image' );
			...
			if ( ! is_array( $image ) || is_null( $post_id ) )
				return new WP_Error( 'invalid', __( 'An error occured when trying to import image: invalid data or Post ID.', 'wpmoviescore' ) );

			$response = self::image_upload( $image['file_path'], $post_id, $tmdb_id, 'backdrop', $image );
			wpmcp_ajax_response( $response );
		}
		...
		public static function set_featured_image_callback() {
			...
			wpmcp_check_ajax_referer( 'set-movie-poster' );
			...
			if ( 1 != wpmcp_o( 'poster-featured' ) )
				return new WP_Error( 'no_featured', __( 'Movie Posters as featured images option is deactivated. Update your settings to activate this.', 'wpmoviescore' ) );
		...
		public static function delete_movies_attachments( $post_id ) {
			...
			// Do nothing
			if ( ! wpmcp_o( 'images-delete' ) && ! wpmcp_o( 'posters-delete' ) )
				return false;
			...
			foreach ( $attached as $a )
				if ( '' != get_post_meta( $a->ID, '_wpmcp_backdrop_related_tmdb_id', true ) ||
				     '' != get_post_meta( $a->ID, '_wpmcp_poster_related_tmdb_id', true ) )
		...
		public static function get_movie_imported_attachments( $type = 'image' ) {
			...
			$args = array(
				'post_type'      => 'attachment',
				'post_status'    => 'inherit',
				'meta_key'       => "_wpmcp_{$type}_related_tmdb_id",
				'posts_per_page' => -1,
		...
		private static function image_upload( $file, $post_id, $tmdb_id, $image_type = 'backdrop', $data = null ) {
			...
			if ( empty( $file ) )
				return new WP_Error( 'invalid', __( 'The image you\'re trying to upload is empty.', 'wpmoviescore' ) );

			$image_type = ( 'poster' == $image_type ? 'poster' : 'backdrop' );

			if ( 'poster' == $image_type ) {
				$size = wpmcp_o( 'poster-size' );
			} else {
				$size = wpmcp_o( 'images-size' );
			}

			if ( is_array( $file ) ) {
				$data = $file;
				$file = WPMCP_TMDb::get_image_url( $file['file_path'], $image_type, $size );
				$image = $file;
			}
			else {
				$image = $file;
				$file = WPMCP_TMDb::get_image_url( $file, $image_type, $size );
			}
			...
			if ( false !== $existing )
				return new WP_Error( 'invalid', __( 'The image you\'re trying to upload already exists.', 'wpmoviescore' ) );
		...
		private static function update_attachment_meta( $attachment_id, $post_id, $image_type, $data ) {
			...
			$tmdb_id    = WPMCP_Movies::get_movie_meta( $post_id, 'tmdb_id' );
			$title      = WPMCP_Movies::get_movie_meta( $post_id, 'title' );
			$production = WPMCP_Movies::get_movie_meta( $post_id, 'production_companies' );
			$director   = WPMCP_Movies::get_movie_meta( $post_id, 'director' );
			$original_title = WPMCP_Movies::get_movie_meta( $post_id, 'original_title' );

			$production = explode( ',', $production );
			$production = trim( array_shift( $production ) );
			$year       = WPMCP_Movies::get_movie_meta( $post_id, 'release_date' );
			$year       = apply_filters( 'wpmcp_format_movie_date',  $year, 'Y' );

			$find = array( '{title}', '{originaltitle}', '{year}', '{production}', '{director}' );
			$replace = array( $title, $original_title, $year, $production, $director );

			$_description = str_replace( $find, $replace, wpmcp_o( "{$image_type}-description" ) );
			$_title       = str_replace( $find, $replace, wpmcp_o( "{$image_type}-title" ) );
			...
			update_post_meta( $attachment_id, '_wpmcp_' . $image_type . '_related_tmdb_id', $tmdb_id );
			update_post_meta( $attachment_id, '_wpmcp_' . $image_type . '_related_meta_data', $data );
		...
		public static function load_posters_link( $content, $post_id ) {
			...
			$content .= '<a id="tmdb_load_posters" class="hide-if-no-js" href="#">' . __( 'See available Movie Posters', 'wpmoviescore' ) . '</a>';
			$content .= wpmcp_nonce_field( 'set-movie-poster', false, false );
		...
		public static function check_for_existing_images( $tmdb_id, $image_type = 'backdrop', $image = null ) {
			...
			$check = get_posts(
				array(
					...
					'meta_query' => array(
						array(
							'key'     => '_wpmcp_' . $image_type . '_related_tmdb_id',
							'value'   => $tmdb_id,
						)
					)
				)
			...
			if ( 'poster' == $image_type && ! empty( $check ) ) {
				foreach ( $check as $c ) {
					$meta = get_post_meta( $c->ID, '_wpmcp_' . $image_type . '_related_meta_data' );
		 ...
		 * 
		 * This is used by WPMCP_Edit_Movies::load_images_callback() to
		...
		public static function fake_jsonify_movie_images( $images, $post, $image_type ) {
			...
			$base_url = WPMCP_TMDb::get_image_url( null, $image_type );
			$json_images = array();
			$i = 0;
